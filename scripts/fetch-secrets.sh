...
# Gateway → kiteclass-core cross-EC2 route URL. Compute in the body (NOT inline in
# the heredoc) so the value is bound before the heredoc expands — EC2 bash 5.2.15
# errored on the inline `${KITECLASS_CORE_URL:-...}` form under `set -u` inside the
# heredoc (the `tee` then failed, leaving /etc/kite/.env stale). Matches the pattern
# of every other heredoc var (DB_*, SEPAY_API_KEY, etc. — computed here, plain-ref'd
# below). 10.0.0.155 = kc-app private IP (update on instance replace).
...
# Value computed in the body above (bound before heredoc expands per EC2 bash fix).
KITECLASS_CORE_URL=${KITECLASS_CORE_URL}
ENVEOF
