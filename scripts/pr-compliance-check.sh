...
check_single_pr() {
  ...
  # Gather all checks
  local ci tests docs audits wave
  ci=$(check_ci "$pr")
  ...
  if $JSON_MODE; then
    ...
    for check in "$ci" "$tests" "$docs" "$audits" "$wave"; do
      local status="${check%%:*}"
...
EOF
  ...
  for label_check in "CI green at merge:$ci" "Tests for new code:$tests" "Business docs updated:$docs" "Required audits:$audits" "Wave completion check:$wave"; do
...
check_range() {
  ...
  echo "═══════════════════════════════════════════════════════════"
  printf "%-6s │ %-4s │ %-7s │ %-6s │ %-8s │ %-6s │ %s\n" "PR" "CI" "Tests" "Docs" "Audits" "Wave" "Score"
  echo "───────┼──────┼─────────┼────────┼──────────┼────────┼──────"
  ...
  for pr in $(seq "$from" "$to"); do
    ...
    printf " #%-4s │  %b  │   %b    │  %b   │   %b     │  %b   │ %s\n" \
      "$pr" "$(fmt_status "$ci")" "$(fmt_status "$tests")" "$(fmt_status "$docs")" \
      "$(fmt_status "$audits")" "$(fmt_status "$wave")" "$score"
  done
