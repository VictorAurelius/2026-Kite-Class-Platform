 ...
 *
 * Source spec: kitehub-pro-v2/screens/dashboard-success.html.
 * Mirror of Wave 30 KC primitive — verbatim (no app coupling beyond brand
 * palette comment, which is just a default).
 *
 * Two surfaces:
 *   - `fireConfetti()` imperative API — call from event handlers
 *     (e.g. after invoice paid, instance deployed). Mounts a transient canvas
...
interface ConfettiOptions {
  ...
  duration?: number;
  /** Optional palette — defaults to KH pro v2 brand mix (sky blue + orange accent). */
  colors?: string[];
}

const DEFAULT_COLORS = ['#0EA5E9', '#22D3EE', '#22C55E', '#F97316', '#EF4444'];
...
function createParticles(count: number, colors: string[], width: number): Particle[] {
  ...
  for (let i = 0; i < count; i += 1) {
    particles.push({
      ...
      color: colors[Math.floor(Math.random() * colors.length)] ?? colors[0] ?? '#0EA5E9',
