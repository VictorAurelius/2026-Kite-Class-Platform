 ...
 *
 * Per `.claude/rules/user-manual-content-standard.md` §3 — URL `/help/p2-owner/{slug}`.
 * Source: `documents/05-guides/user-manual/p2-owner/{slug}.md`
 *
...
export default async function P2OwnerManualPage({ params }: PageProps) {
  const { slug } = await params;
  const page = await getManualPage(PERSONA, slug);
  if (!page) {
    notFound();
