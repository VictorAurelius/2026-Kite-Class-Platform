 ...
 *
 * Per `.claude/rules/user-manual-content-standard.md` §3 — URL `/help/platform-admin/{slug}`.
 * Source: `documents/05-guides/user-manual/platform-admin/{slug}.md`
 *
...
export default async function PlatformAdminManualPage({ params }: PageProps) {
  const { slug } = await params;
  const page = await getManualPage(PERSONA, slug);
  if (!page) {
    notFound();
