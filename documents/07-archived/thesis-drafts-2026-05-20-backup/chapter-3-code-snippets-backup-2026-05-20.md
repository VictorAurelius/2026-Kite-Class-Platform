---
title: Wave 102.5 backup — Ch.3 5 code snippet analysis (removed per Item 9a)
status: archived
created: 2026-05-20
related-wave: 102.5
audience: dev
...
# Backup — 5 code snippet analysis (Chương 3 cũ §3.2-§3.8)

**Trạng thái:** Archived 2026-05-20 per Wave 102.5 Bucket E Item 9a.

**Lý do remove khỏi Chương 3 main flow:** Khung-chuẩn UTC §2.2 yêu cầu Chương 3 trình bày "Kết quả triển khai sản phẩm" và "Kiểm thử (kế hoạch + ca tiêu biểu)" — không phải code snippet analysis. 5 code snippets gốc minh họa design pattern (JWT auth, RLS, Outbox, Controller, Next.js) phù hợp với nội dung kỹ thuật sâu nhưng vượt scope khung-chuẩn cử nhân CNTT.

**Mục đích lưu backup:** Nội dung kỹ thuật sâu vẫn có giá trị tham khảo cho dev team. Lưu trong `documents/08-thesis/` để có thread liên tục, thay vì archive sang `07-archived/`.

**Mapping section gốc (từ Chương 3 trước khi rework Wave 102.5):**

- §3.2 Phạm vi năm đoạn mã đại diện
- §3.3 JWT Authentication Flow tại Gateway — pattern Chain of Responsibility + Trust boundary
- §3.4 Multi-tenant Query với RLS NULL Force-Fail — pattern AOP + Default-deny + Session GUC
- §3.5 Email Worker Outbox Pattern — pattern Transactional Outbox + Scheduled dispatcher + Backoff
- §3.6 Beta Access Controller Cluster — pattern 3-Tier layering + `@PreAuthorize` + Audit aspect
- §3.7 Frontend Next.js App Router Page — pattern Server Component + Composition + Separation of concerns
- §3.8 Tóm tắt Chương 3 (phiên bản cũ)
