...
**G3 production-parity preview:** Sau AWS restore — walk full gateway JWT→header chain (real PARENT login) + multi-tenant parent isolation (parent tenant A ↛ child tenant B) + audit log (`parent_read_audit_log` ghi đúng mỗi facet read, KHÔNG ghi khi consent-blocked) + PDPL consent enforcement production.
