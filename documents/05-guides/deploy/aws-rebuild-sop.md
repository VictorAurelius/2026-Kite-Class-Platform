...
| Gate | Điều kiện pass | Bước kiểm tra | Hậu quả nếu fail |
|---|---|---|---|
| **Gate 1: Account unban** | `aws sts get-caller-identity` trả về AccountId hợp lệ | Bước 1-2 | STOP toàn bộ quy trình; chờ Support case |
| **Gate 2: CloudTrail logging** | `aws cloudtrail get-trail-status --name kitehub-main` → `IsLogging = True` | Bước 5 | KHÔNG proceed Bước 6; apply CloudTrail targeted apply trước |
| **Gate 3: Infrastructure healthy** | EC2 `running` + RDS `available` + ALB `active` | Bước 6 verify | KHÔNG proceed Bước 9 deploy; debug terraform apply |
| **Gate 4: Application smoke** | All health checks PASS + admin login HTTP 200 + JWT | Bước 11 | Debug ứng dụng; không flip gaps DONE |
| **Gate 5: Email flows** | SES `Enabled = true` + identities `verified` | Bước 12 | Email features không hoạt động; dùng Resend fallback hoặc đợi SES approval |
...
|---|---|---|
| Phase 1 — SOP draft (Wave 103 Bucket F) | Tài liệu này: 13 bước outline + 5 gates + 8 failure modes | ✅ 70% PARTIAL |
| Phase 2 — Commands verified (Wave 104) | Chạy thực tế từng lệnh sau AWS restore; fill in real IDs/ARNs; validate gates bằng actual CLI output | ⏳ Defer Wave 104 |
| Phase 3 — Automation script (Wave 105+) | `scripts/aws-rebuild-sop.sh --dry-run` auto-check gates | ⏳ Defer Wave 105+ |

**Lý do defer Phase 2:** AWS account `906286017800` vẫn suspended tại thời điểm soạn thảo (2026-05-22). Các lệnh AWS trong tài liệu này ở outline level với placeholder commands. Execution-validated commands sẽ được cập nhật sau khi account restore (per GAP-612 unblock).
