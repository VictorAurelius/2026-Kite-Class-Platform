...
**Status:** 🟢 ACCEPTED (2026-04-18, GAP-103)
...
Mọi production artifact đóng gói thành Helm charts (`infrastructure/helm/`). Không lock vào AWS-specific services (CloudFormation, Lambda, RDS Proxy) nếu có alternative portable (K8s Job, StatefulSet, pgbouncer). Test Helm chart trên cả AWS EKS và Oracle Cloud OKE.

**Why:** Options open — có thể migrate cloud trong 1 tuần thay vì 3 tháng nếu vendor lock-in.

### 1.2 Cost-conscious Dual-cloud
**Staging** chạy Oracle Cloud Free Tier (ARM Ampere A1, always-free 4 OCPU / 24 GB RAM). **Production target** là AWS EKS spot/reserved hoặc Oracle OKE tùy pilot economics. Development local qua Docker Compose.

**Why:** Graduation thesis budget = 0 USD. Oracle Free Tier cho phép 24/7 staging. AWS commit chỉ khi có paying customer #1.
...
| **Staging** | Oracle Cloud Free Tier (ARM Ampere A1) | `kite-staging.oraclecloud` VM | Zero monthly cost, 24/7 availability for demo |
| **Production (target)** | AWS EKS (preferred) hoặc Oracle OKE | AWS ap-southeast-1 (Singapore) / Oracle AP Mumbai | Latency VN, pricing tier flexibility |
| **IaC** | Terraform | `infrastructure/terraform-aws/`, `infrastructure/terraform-oracle/` | Multi-cloud abstraction |
