...
- **2026-05-07** (addendum, Wave 42 closure) — **CI build matrix locked amd64-only Phase 1 BETA.** Phase 3 first OIDC trigger (`v0.9.0-beta-staging.1`, run #25527705091) failed because `docker-build-push.yml` push step declared `platforms: linux/amd64,linux/arm64` but 7 Java services use `maven:3.9-eclipse-temurin-{17,21}-alpine` base images — Docker Hub publishes amd64-only manifest. PR #1004 dropped arm64 (single-line workflow change, line 284). Decision aligns CI matrix với EC2 architecture: Phase 1 BETA = t3.medium amd64 → CI matrix = amd64 only. arm64 restore deferred Phase 2+ Graviton consideration (would require base image swap to `eclipse-temurin:*-jdk-alpine` + manual maven install OR drop `-alpine` suffix). Failure mode F5 documented in `documents/05-guides/deploy/phase-3-image-push.md`. Per `incident-to-rule-pipeline.md` premature-rule guard: 1st recurrence — no formal rule yet; escalate Tier 2 (CI script + rule) if 2nd recurrence (Graviton migration likely surfaces).
