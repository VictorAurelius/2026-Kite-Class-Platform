...
| 2c | subscription | `APP_BASE_URL` (`kitehub.app.base-url`) | `application.yml` `kitehub.app.base-url` + `EmailServiceClient` / `OwnerNotificationDispatcher` / `DomainService` @Value | https://kitehub.me | docker-compose env (default already prod-correct kitehub.me; pinned in application-production.yml) | ✅ Added (GAP-1414) | Canonical public app base-url for ~25 email + owner-notification + domain-backup links (was hardcoded .com/.me/.vn drift). Path appended per link; tenant subdomain links interpolate {subdomain} into host (`https://{sub}.kitehub.me`) |
