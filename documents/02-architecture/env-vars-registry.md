...
| 22 | kiteclass-core | `SPRING_DATASOURCE_URL` (KC own DB) | `kiteclass-core/application.yml:26` | `jdbc:postgresql://localhost:5433/...` | **`docker-compose.kc.yml` `environment:` block** (override env_file) | ✅ Overridden (handoff 2026-06-18 Gap 1) | kiteclass-core uses its OWN DB `kiteclass_shared`, NOT subscription's `kitehub` DB. Shared `/etc/kite/.env` (fetch-secrets.sh row 10) writes a single `SPRING_DATASOURCE_URL` pointing at `kitehub` → connecting there sees subscription's Flyway history → checksum mismatch + V76 fail → crash-loop. KC compose pins `jdbc:postgresql://<rds>:5432/kiteclass_shared` explicitly (env_file username/password reused — RDS master user has access). KC stack = `docker-compose.kc.yml` on kc-app EC2 (NOT in `docker-compose.production.yml`). |
| 23 | kitehub-subscription | `DATABASE_ADMIN_URL` / `DATABASE_ADMIN_USERNAME` / `DATABASE_ADMIN_PASSWORD` / `DATABASE_MASTER_HOST` / `DATABASE_MASTER_PORT` | `kitehub-subscription/application.yml:254-259` (read by `DatabaseConnectionService.java:22`) | `localhost:5433/postgres` / `postgres` / empty | `fetch-secrets.sh` → `/etc/kite/.env` | ✅ Overridden (handoff 2026-06-18 Gap 3) | `application-production.yml` sets `database.lifecycle.enabled=true` → `DatabaseProvisioningService` opens an admin connection to CREATE per-tenant DBs on beta-signup. Defaults point at `localhost:5433` → Connection refused → beta-signup 500. fetch-secrets writes RDS-master admin conn (`postgres` maintenance DB, master creds which have CREATE DATABASE). **Supersedes prior acceptable-default classification** ("psql direct") — provisioning uses it programmatically, not just manual SQL. Open design question (handoff): is per-tenant physical-DB provisioning still wanted vs shared-DB model? → separate gap. |
...
| ~~`DATABASE_ADMIN_URL` / `DATABASE_MASTER_*`~~ | ~~localhost~~ | **MOVED to overridden table row 23 (handoff 2026-06-18 Gap 3)** — prior "psql direct" assumption was wrong: `database.lifecycle.enabled=true` makes `DatabaseProvisioningService` use the admin conn programmatically at beta-signup, so it MUST point at RDS master (now written by fetch-secrets.sh). No longer an acceptable-default. |
