...
| 16 | kitehub-subscription, kitehub-admin | `TOTP_ENCRYPTION_KEY` | `kitehub-subscription/application.yml:109` (TOTP secret AES-256 encryption at rest) | dev default 36 bytes (`dev-key-32-chars-pad-pad-pad-pad-pad`) | fetch-secrets.sh → /etc/kite/.env (Wave 81 Bucket F PR #1389) | ✅ Overridden | AWS Secrets Manager `kitehub/production/totp-encryption-key` (created 2026-05-15 manual). Wave 72b GAP-516 `TotpSecretCipher.@PostConstruct` fail-fast guard mandate non-dev-default + ≥32 bytes trong production profile |
