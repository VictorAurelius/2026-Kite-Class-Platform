...
| BR-AUTH-JWT-002 | Shared `JWT_SECRET` ≥ 64 bytes, fail-fast | Key đọc từ `jwt.secret` / env `JWT_SECRET` (cùng giá trị gateway). `@PostConstruct init()` ném `IllegalStateException` nếu secret < 64 bytes (yêu cầu key 512-bit cho HS512). **Code:** `AuthTokenService.java` `init()`. **Lưu ý:** gateway check chỉ ≥32 bytes — gate thực tế ở Core ≥64; theo dõi follow-up P2 đồng bộ. |
