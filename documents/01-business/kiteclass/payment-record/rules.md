...
- **Compliance check:** **Considered (self-assessed, counsel pending GAP-156 AC-D)** — per `documents/00-brd/compliance-checklist.md` L2 (+ AML overlay): **Luật Quản lý Thuế 2019** Art 18 (financial-record retention 10 năm, append-only BR-PAYMENT-RECORD-010); **Nghị định 123/2020/NĐ-CP** (e-invoice — deferred Phase 1.5+ via GAP-185 MISA MeInvoice partnership); **Luật Phòng chống rửa tiền 2022** (cash-receipt recording — `CASH` method BR-PAYMENT-RECORD-001; large-cash-transaction reporting thresholds to review with counsel); plus **Nghị định 13/2023/NĐ-CP (PDPL)** Art 11 (tamper-proof audit trail). Append-only design is a self-assessed control, not counsel-confirmed — no tax-advisor / AML-counsel verification yet.
