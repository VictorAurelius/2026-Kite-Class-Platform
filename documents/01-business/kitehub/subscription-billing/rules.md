...
| SUB-23 | Pending-payment TTL + dunning | `Payment PENDING` có TTL `kitehub.subscription.pending-payment-ttl-days: 7` → quá hạn auto-EXPIRED + giải phóng `pendingPaymentId`. Trong khi chờ admin confirm VietQR: chuỗi nhắc hoàn tất CK + grace-period reminder "còn X ngày trước suspend". Wave kitehub-biz-100. | GAP-1259; SubscriptionExpirationChecker + scheduler |
| SUB-24 | Involuntary-churn (PAID hết grace) | Subscription ACTIVE hết hạn + qua grace 3 ngày (SUB-04) mà chưa thanh toán → auto-suspend instance + đánh dấu involuntary churn (phân biệt voluntary cancel SUB-12/13). Wave kitehub-biz-100. | GAP-1260; SubscriptionExpirationChecker.suspendExpiredSubscription |
