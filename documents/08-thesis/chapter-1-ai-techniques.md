---
title: Chương 1 §1.4 — Kỹ thuật AI tích hợp trong nền tảng đề xuất
chapter: 1
...
# Chương 1 §1.4 — Tổng quan kỹ thuật AI tích hợp trong nền tảng đề xuất

## 1.4 Tổng quan kỹ thuật AI tích hợp trong nền tảng đề xuất

### 1.4.1 Bối cảnh AI trong giáo dục SaaS

Trí tuệ nhân tạo, đặc biệt là các mô hình ngôn ngữ lớn (LLM) như GPT-3 [11] và các mô hình diffusion sinh ảnh như Stable Diffusion [10], đã tạo ra cuộc cách mạng trong nhiều ngành công nghiệp giai đoạn 2022-2026. Ngành giáo dục không ngoại lệ. Theo báo cáo 6Wresearch [8], thị trường EdTech Việt Nam dự báo tăng trưởng CAGR 12-15% giai đoạn 2024-2030, trong đó AI-powered features là yếu tố khác biệt cạnh tranh quan trọng cho SaaS phân khúc tier trung và cao.
...
### 1.4.2 Phương pháp 1 — AI Branding (text-to-image generation)
...
#### 1.4.2.1 Kiến trúc kỹ thuật AI Branding
...
#### 1.4.2.2 Kỹ thuật prompt engineering
...
Tham khảo nghiên cứu prompt engineering trong [11] (Brown et al., GPT-3 few-shot learning) và [12] (LLaVA — visual instruction tuning) cho phương pháp luận xây dựng prompt hiệu quả cho multimodal models. Tài liệu thực hành prompt engineering từ tài liệu API LLM thương mại [24] cũng cung cấp pattern cụ thể về negative prompting (explicit `NO text` / `NO mascots`) — pattern này được áp dụng trực tiếp trong template ở trên để tăng first-attempt PASS rate.
...
Lý do chọn Stable Diffusion XL [10] làm primary: balance tốt giữa quality + cost + latency. SDXL Turbo làm fallback khi primary rate-limited hoặc lỗi 5xx. Tránh DALL-E 3 vì cost cao 30 lần SDXL, không phù hợp budget giai đoạn thử nghiệm với target $0 infrastructure cost (Free Tier AWS + Replicate free credits).

#### 1.4.2.4 Cost analysis
...
### 1.4.3 Phương pháp 2 — AI Quality Gate (content safety + brand fit)
...
#### 1.4.3.1 Multi-layer gate strategy
...
### 1.4.4 Phương pháp 3 — AI techniques roadmap (giai đoạn mở rộng)
...
#### 1.4.4.1 Chatbot hỗ trợ học viên (giai đoạn mở rộng)

**Mô hình:** LLM API thương mại cost-efficient tier (Anthropic / OpenAI), với context tenant-specific (course catalog + FAQ + lịch học).

**Architecture:** RAG (Retrieval-Augmented Generation) [25] dùng PostgreSQL pgvector extension cho vector search course content + FAQ embeddings. Khi học viên hỏi "Lớp Anh ngữ 5A1 học vào thứ mấy?", system retrieve relevant context từ tenant database thì feed vào LLM thì generate response Vietnamese natural.
...
#### 1.4.4.2 Auto-grading bài tập (giai đoạn mở rộng)
...
#### 1.4.4.3 Personalized learning path (giai đoạn GA)

**Mô hình:** Multi-modal LLM (LLaVA [12] hoặc successor) cho phân tích visual content (homework photos, video bài giảng) kết hợp với student performance data.
...
**Lý do defer:** cần dataset student performance đủ lớn (≥1 năm operation × 1000+ students) + đảm bảo PDPL compliance cho cá nhân hóa (cần explicit consent từ phụ huynh per [2] PDPL Art 11).

### 1.4.5 AI development methodology

KiteHub áp dụng test-driven development (TDD) [14] và domain-driven design (DDD) [26] cho AI feature development, tránh approach "ship first, fix later" thường thấy ở các AI startup. Cụ thể:
...
Theo Luật Bảo vệ Dữ liệu Cá nhân Việt Nam 2023 [2] và Nghị định 13/2023/NĐ-CP [4], xử lý dữ liệu cá nhân bằng AI yêu cầu:
...
#### 1.4.6.3 Transparency với người dùng
...
<!-- §7 Kết luận chương 1 phần 2 — removed Wave 102.5 Item 5 per khung-chuẩn G15 renumber Ch.1 strict 1.X.Y.Z. Conclusion content backed up to chapter-1-conclusion-backup-2026-05-20.md. Single Ch.1 conclusion at end of vn-law-methodology.md §1.7 trim per Wave 102.5 per UTC convention (1 chương 1 conclusion). -->
