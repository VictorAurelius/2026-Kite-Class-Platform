...
> **Cập nhật 2026-06-19 (tách G3-config khỏi G3-infra):** config-bug campaign #2492-2496 lộ một lỗ hổng — G2★ chạy trên **1 Docker host local** KHÔNG tái hiện topology **3-EC2 production** (cross-host private IP, SG self-ref, nginx per-tenant proxy, S3 IAM-role, EC2 bash `set -u` quirk). Các config-bug đó (`KITECLASS_CORE_URL`/`INTERNAL_API_URL` trỏ private IP, routing đa-host) **về cấu trúc không thấy được** ở G2★ đơn-host → rơi vào **vùng KHÔNG gate nào** → chỉ lòi khi deploy AWS, fix từng cái một. Sửa: tách **G3 = G3-config + G3-infra**. **G3-config** = config-parity đa-host verify bằng smoke gate (`scripts/smoke-prod-config.sh`) + registry single-source-of-truth (`documents/05-guides/deploy/prod-deploy-config-registry.md`), chạy SAU `terraform apply` + push image — biến "fix config từng cái" thành "smoke → danh sách fail → batch fix". **G3-infra** = chỉ TLS/LB/wildcard-cert/real-DNS thật-AWS. Per `local-fix-production-parity-check.md` §2.5 (value-resolution dimension).
...
| **G3-config — Deploy-time config parity** (multi-host topology, deploy-gated) | Claude + Dev | Config-parity đa-host KHÔNG tái hiện được trên 1 Docker host local: cross-EC2 URL (`KITECLASS_CORE_URL`/`INTERNAL_API_URL` trỏ private IP host khác), SG self-ref rule, nginx `/api` proxy per-tenant + app block, S3 IAM-role, secret-fetch (`/etc/kite/.env` keys + EC2 bash `set -u` heredoc quirk), prod-profile env (`OTEL_SDK_DISABLED`...). Verify bằng **smoke gate `scripts/smoke-prod-config.sh`** (catalog-then-batch: chạy hết check → bảng PASS/FAIL → batch fix, KHÔNG fix từng cái) + registry `documents/05-guides/deploy/prod-deploy-config-registry.md` single-source-of-truth. Chạy SAU `terraform apply` + push image, TRƯỚC `✅ THÔNG (production)`. **Đây là tầng config-bug campaign #2492-2496 lòi tới deploy vì KHÔNG gate nào cover** — G2★ local đơn-host về cấu trúc không thấy cross-host routing bug. KHÔNG block `🟢 THÔNG (local)` |
| **G3-infra — AWS infra parity** (residual, AWS-gated) | Claude + Dev | Phần parity **không local-reproduce-được KỂ CẢ multi-host emulation**: TLS thật, ALB/LB routing, wildcard-cert `*.kitehub.me`, real-DNS resolution. Verify trên AWS stack (gated GAP-612 — stack đang stopped). **KHÔNG block `✅ THÔNG` ở local-verifiable layer**; flow đạt G1+G2★ → `🟢 THÔNG (local)`, G3-config + G3-infra là checkpoint AWS-deploy riêng |

Flow `🟢 THÔNG (local)` khi **G1 + G2★** PASS (full functional + production-access-mode parity local). **`✅ THÔNG (production)`** khi thêm **G3-config** (deploy-time config-parity smoke `scripts/smoke-prod-config.sh` PASS) + **G3-infra** (TLS/LB/wildcard-cert/real-DNS) trên AWS. G1 đạt → `🔄 walk-pass-pending-human` chờ G2★.
...
| KH-3 | Subscription create + trial→paid migration | 3 | ✅ G1+G2 PASS — chờ G3 production parity (AWS) — 2026-06-09 (G2 real-transfer SePay: login test-8 → upgrade → QR MB 10k → chuyển khoản thật → SePay webhook auto-confirm → PREMIUM ACTIVE + activation email; Bug E/F/D verified live) | [wave-2026-06-04-flow-kh3](../waves/wave-2026-06-04-flow-kh3-subscription-trial-paid.md) + [G2 recipe](../../05-guides/operations/2026-06-04-g2-recipe-kh3-subscription.md) | ✅ GAP-942 DONE; ✅ GAP-1085 (email double-publish) + GAP-1086 (email content VN/txt/domain) + GAP-1087 (Bug D memo==txnRef) DONE (PR #2275); GAP-974 (activated-email upgrade-flow P1) + GAP-1088 (email domain/txt sweep P2) OPEN |
| KH-4 | **Subscription upgrade manual VietQR + admin confirm** | — | ✅ THÔNG (G1) | — | (GAP-914 fixed) |
| KC-1 | Tenant provisioning + lifecycle + settings | 4 | 🟢 THÔNG (local) — G2★ human browser walk settings/branding PASS 2026-06-17 trên g2walk (nip.io); GAP-1467 color-picker dual-register fixed inline + verified live. Residual GAP-952 (CloudWatch AWS-deferred) PARTIAL không block local. (provisioning-1 closure walk 2026-06-07: saga LIVE + 3/7 sub-gaps DONE — GAP-947 settings + GAP-953 admin-retry + GAP-954 PDPL purge; 3 bugs fixed live) — 2026-06-17 | [wave-2026-06-04-flow-kc1](../waves/wave-2026-06-04-flow-kc1-tenant-provisioning-settings.md) | ✅ GAP-1467 DONE (color picker); GAP-947/953/954 DONE; ✅ GAP-945/946/948/949 DONE (2026-06-07, Wave p0-prov-1; GAP-946 real-impl → GAP-1055 Phase 1.5); GAP-952 (CloudWatch AWS-deferred) PARTIAL |
...
Status: ⬜ chưa walk · 🔄 G1 pass chờ human (G2★) · 🟢 THÔNG (local) = G1+G2★ · ✅ THÔNG (production) = +G3-config (smoke deploy) +G3-infra (AWS). Legacy rows `✅ THÔNG (G1+G2+G3)` = THÔNG (local) theo mô hình cũ; G3-infra AWS-gated chung GAP-612.
...
- **2026-06-17 (KC-1 G2★ human walk PASS → 🟢 THÔNG (local); GAP-1467 color-picker fixed live):** Human browser walk production-accurate (tenant `g2walk`, nip.io) qua settings → branding FULL PASS. Bắt + fix inline **GAP-1467** (P2): color-picker dual-register — mỗi field màu render 2 `<Input>` cùng spread `{...register('X')}` (swatch + hex) → react-hook-form binding hỏng, pick màu mới KHÔNG cập nhật submit (ref stale) → PUT persist màu CŨ `#3B82F6`. Fix `branding-settings.tsx`: 3 cặp input màu (primary/secondary/accent) chuyển dual-`register` → controlled (`value={watch('X')}` + `onChange → setValue`, single source of truth). **Verify live:** server-side `GET /api/v1/settings/branding` trả màu MỚI persist `#3bf79f/#5ff7d8/#b71053` (vs `#3B82F6` cũ); PUT 08:13:51 HTTP 200 no validation error; swatch↔hex sync 2 chiều; docker rebuild production `next build` PASS + `kiteclass-frontend` container recreate healthy. GAP-1467 DONE + git mv closed/. KC-1 §4 status `🔄 walk-pass-pending-human` → **🟢 THÔNG (local)**; residual GAP-952 (CloudWatch AWS-deferred) PARTIAL không block local.

- **2026-06-16 (agent headless browser-walk batch — 5 flow còn lại: KC-1/2/3 + KH-7 PASS, KC-8 blocked):** Claude headless Playwright walk (nip.io production-accurate `:3000` KC / `:3001` KH) 5 flow chưa browser-rewalk, 5 agent Opus song song (3 walk + 2 pre-walk sim) + coordinator inline. **Verdicts:** KC-1 tenant-settings ✅ PASS 0 product bug; KC-2 staff-invite ⚠️ mostly-pass (RBAC solid; **FM-1 fixed+verified** — email link localhost:3001); KC-3 academic ✅ PASS (BUG-KC3-2 Invalid Date fixed); KH-7 domain ✅ FULL PASS (CustomDomainTab FE tồn tại — recipe cũ sai; GAP-1023 IDOR nay 403). **KC-8 parent ⛔ BLOCKED** — parent seed thiếu hẳn (GAP-1457) + FE facet pages mock (GAP-1458 Phase 1.5). Code fix PR #2456: formatDate→'—' cross-flow + FM-1 base-url local+prod parity. 6 gap filed (GAP-1457..1462). Evidence: [`2026-06-16-walk-{kc1,kc2,kc3,kh7}-*.md`](../../04-quality/audits/rst-html/). Pre-walk predictions phần lớn refuted. **Status §4:** 4 flow giữ `🔄 walk-pass-pending-human` — headless walk = agent-G1-browser (KHÔNG thay human G2★); human spot-check UX vẫn khuyến nghị trước flip ✅ THÔNG. KC-8 UNBLOCKED 2026-06-16: GAP-1457 seed DONE (dev-seed-parent-kc8.sql, verified gateway login+facets+IDOR 403); GAP-1458 FE facet wiring PARTIAL (Phase 1.5). Wave-plan §11 "parent Phase-2-gated" STALE — auth-1 (GAP-725/1122) đã ship parent login.

- **2026-06-14 (G1 FE browser walk — RBAC + LMS: 12/12 walkable PASS, SSO BLOCKED):** Claude headless Playwright walk FE thật (`skytest.127.0.0.1.nip.io:3000` subdomain Host per `g1-browser-walk-before-flip.md` §3.1) sau rebuild `kiteclass-frontend`+`kitehub-frontend` (image cũ 3-4 ngày). Rung TRÊN BE-contract walk (rung dưới gateway curl). Evidence: [`2026-06-14-g1-fe-browser-walk.md`](../../04-quality/audits/rst-html/2026-06-14-g1-fe-browser-walk.md). Verdict: (1) RBAC role-redirect 4 role (ADMIN→/dashboard, TEACHER→/teacher/dashboard, STUDENT→/student/today, PARENT→/parent) ✅; (2) RoleGuard cross-role 3 hướng chặn ✅; (3) `/admin/roles` render 5 mẫu vai trò ✅; (4) LMS teacher content-tab + guest catalog (Test Course + paywall CTA) ✅; (5) LMS student learning (enrolled course card) + assignments ✅. **0 FE-runtime bug** (render/blank/redirect-loop/wrong-API/missing-nav). Console clean + 0 net 4xx mọi flow. **1 flow BLOCKED:** SSO KH→KC — không drive được vì thiếu KiteHub owner credential local (KH auth = kitehub-subscription tách biệt KC tenant-auth) → **GAP-1305**. **Lưu ý positive:** lần đầu student-learning empty do Claude setup error (credential map cross-tenant student id=4 thuộc tenant `0edaee10` ≠ skytest) → RLS ĐÚNG chặn cross-tenant → re-map student id=164 skytest → PASS (RLS isolation hoạt động đúng). **VẪN KHÔNG flip gap → DONE / không flip `walk-pass-pending-human`** — human G2★ bắt buộc (mutation assign-role/CRUD-content/mark-complete/submit + SSO + STAFF-scope chưa walk, defer G2★). 7 gap PARTIAL (1138/1119/1113/1277/1285/1297/1298) thêm note G1-FE, giữ PARTIAL.

- **2026-06-14 (G1 gateway BE-contract walk — RBAC + LMS + SSO: 6/6 flows BE-verified PASS):** Claude agent G1 walk qua gateway `:9000` minted HS512 JWT (KHÔNG phải browser-walk). Evidence: [`2026-06-14-g1-runtime-walk-rbac-lms.md`](../../04-quality/audits/rst-html/2026-06-14-g1-runtime-walk-rbac-lms.md). Verdict per flow: (1) RBAC RoleGuard cross-role 403 đủ 4 chiều ✅; (2) RBAC Bucket D assign/list ✅ sau fix; (3) STAFF coverage allow enrollment/invoice/attendance + deny payroll/branding 403 ✅; (4) LMS teacher authoring + guest catalog ✅ sau fix; (5) LMS student lesson-player + progress + enrollment-isolation ✅ sau fix; (6) SSO issue→exchange→replay-401→CSRF-415 ✅ FULL PASS. **2 blocking bug fixed inline + G1 re-walk PASS:** GAP-1297 (LmsController/LessonProgressController đọc `X-User-Id`=UUID as Long → 400 mọi authenticated LMS GET; fix `X-User-Reference-Id`) + GAP-1298 (RoleService assign/list `LazyInitializationException` 500; fix `@Transactional(readOnly=true)`). **1 security PoC** (STUDENT token + spoof `X-Teacher-Id` → 201 LMS authoring) appended vào GAP-798 (không file gap mới — duplicate). **VẪN KHÔNG flip `walk-pass-pending-human`**: G1 này là BE-contract (FE image stale 2-day pre-session + chưa rebuild) → browser-real walk các trang FE mới = G2★ human per `g1-browser-walk-before-flip.md` §1. BE-contract đủ để mở G2★ theo 4 recipe.

- **2026-06-16 (Phase-3 fix-wave — 14 product bug từ Phase-2 walk → G2 PASS, 14 gaps DONE):** Wave `wave-flow-fix-1-phase3` fix 14 product bug (GAP-1435..1448) surfaced từ Phase-2 browser walk 8 flow, span 7 flow (KH-5/6/8/9/10 + KC-10/12). 5 bucket disjoint (4 agent Opus + coordinator inline) → 1 integration PR #2451. Coordinator BE/SSR verify: GAP-1439 (DSAR 400 not 401) + GAP-1444 (beta-status SSR live qua INTERNAL_API_URL) + GAP-1437 (validation test). **Human G2 browser walk 2026-06-16: KH (`:3001`) + KC (`sky-education.127.0.0.1.nip.io:3000` per §3.3) PASS toàn bộ** → 14 gaps flip DONE + git mv closed/. Meta hardening cùng wave: detector `check-walk-recipe-access-mode.sh` (g1-browser-walk §3.1/§3.2 recurrence #2) + §3.3 canonical KC local-env access recipe (slug-source + nip.io/.kiteclass.local + GAP-1067 restart gotcha). Residual: GAP-1455 (prod INTERNAL_API_URL AWS-blocked) + GAP-1456 (~34 recipe access-mode remediation WARN-grandfathered) + GAP-1447 SSO KC↔KH future. 7 flow status giữ `🔄 walk-pass-pending-human` (residual non-Phase-3 gaps mỗi flow vẫn pending per row §4).
- **2026-06-14 (RBAC + LMS + SSO + KC-9 student-shell — G2 recipes ready, G1 browser-walk + G2 pending):** Wave RBAC-LMS shipped code+tests cho tầng RBAC role-shell (GAP-1119 role routing + Bucket D assign-UI), KC-9 student-auth (GAP-1277 BE + student-shell FE), LMS frontend (GAP-1113 Increment A teacher-authoring/guest-catalog + Increment B student-player/assignment), student-self enrollment endpoint (GAP-1285), cross-product SSO KH→KC (GAP-1138 ADR-040 Option A). 4 G2 handoff recipe MD shipped (per `g2-handoff-md-mandate.md`) để dev walk production-accurate browser: [rbac-role-shell](../../05-guides/operations/2026-06-14-g2-recipe-rbac-role-shell.md) · [lms-teacher-catalog](../../05-guides/operations/2026-06-14-g2-recipe-lms-teacher-catalog.md) · [lms-student](../../05-guides/operations/2026-06-14-g2-recipe-lms-student.md) · [sso-kh-kc](../../05-guides/operations/2026-06-14-g2-recipe-sso-kh-kc.md). **Status NOT flipped to `walk-pass-pending-human`**: các gap còn PARTIAL với "G1 browser-walk + G2-walk pending" (per `g1-browser-walk-before-flip.md` §1 — flip cần G1 browser-walk evidence chưa có). KC-9 student portal (§4 row ⛔ DEFERRED Phase 2) được **pull forward** Phase 1 (student-auth + student-shell shipped) nhưng giữ status chính xác tới khi G1 browser-walk + G2 human walk PASS. Recipes = handoff artifact enabling human G2 walk.

- **2026-06-12 (landing-100 G2★ PASS — gate model validated):** Human walk production-accurate nip.io PASS (3 tenant theme riêng + empty-state + mobile + sad-path SUSPENDED/unknown-subdomain) — lần đầu G2★-absorbs-G3-functional (§1, 2026-06-11) chạy trọn một flow. GAP-1077 DONE (Host-resolution middleware verified bằng đúng access-mode). G3-infra residual (TLS/wildcard/real-DNS) giữ PENDING GAP-612. Wave landing-100 flip complete (§7.2 reconciliation).
