...
- **2026-06-05 (GAP-983 fix attempt + revert):** Thử fix v1 `@Filter` re-declare 4 entity → rebuild + live re-test PARTIAL (teacher blocked but 500; getClass `@Transactional` vẫn leak). ROOT CAUSE: OSIV off (`open-in-view: false`) + `@Transactional` methods mở session riêng mà `TenantFilterInterceptor.enableFilter` không reach. `@Filter` cần nhưng KHÔNG đủ — phải fix filter-enablement trên txn session (platform-wide infra change + 58 entity sweep + full IT). Reverted per `release-fix-retry-budget.md` §3 (không ship half-fix); root-cause + 4-layer fix plan documented GAP-983. Stack rebuilt clean. **Fix = dedicated security wave (ties GAP-746/749/362).**
