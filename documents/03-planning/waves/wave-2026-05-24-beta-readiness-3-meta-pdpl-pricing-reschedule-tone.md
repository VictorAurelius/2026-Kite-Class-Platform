---
title: Wave beta-readiness-3 — META env-coverage + PDPL consent API + Pricing PER_HOUR + Reschedule + Email tone cross-cut
wave: 3
...
# Wave beta-readiness-3 — META env-coverage + PDPL consent API + Pricing PER_HOUR + Reschedule + Email tone cross-cut

**Mục tiêu:** Ship 5 buckets đóng META P0 (GAP-508 Phase 2 + Phase 3) + Compliance P1 (GAP-353b consent API + hash chain) + 2 Business-Logic P0 greenfield (GAP-292 pricing + GAP-292b payment recording paired; GAP-291 reschedule + email fallback) + 1 META cross-cut force-multiplier (email tone matrix Mustache helper) cho Phase 1 BETA exit gate ≥ 80 ngày càng gần.

**Khởi sự:** Wave beta-readiness-2 ship 4/4 (PR #1767/1768/1769/1771); 3 outside-in agents 2026-05-24 (persona simulation + VN edu benchmark + failure-mode matrix) đã surface 7 P0 + 10 P1 cells cross-bucket. User 4-Q decisions chốt: (a) PDPL deadline giữ 2026-07-01 per CLAUDE.md, (b) Pricing taxonomy = PER_HOUR primary, (c) Bucket C ghép GAP-292b paired.

**Thời gian ước tính:** ~14h (2-3 phiên) — 5 bucket scope, Bucket A sequence trước, B/C/D parallel, E cross-cut sau khi A code lands.

**Tag scheme:** per `.claude/rules/wave-tag-numbering-convention.md` §2 — `beta-readiness-3` counter 3, descriptor `meta-pdpl-pricing-reschedule-tone`.
...
- Phase 1 BETA gate quality ≥ 80 — Compliance + Business-Logic P0 closure
...
- Cross-cut: tất cả 4 buckets B/C/D/E đụng email templates → cần shared persona tone matrix Mustache helper (META force-multiplier per `meta-gap-priority.md` §3)
...
- ❌ Ship GAP-353 hash chain Phase 2 sau Wave br-3: persona Agent 1 P0-3 explicit — PDPL Art 11 audit fail risk; cross-bucket Agent 3 Cell 6 lifecycle handler mandate
...
- B đụng `kitehub/kitehub-subscription/src/main/java/.../consent/` (new package) + `kitehub-subscription/src/main/resources/db/migration/V26__create_consent_record.sql` + `packages/shared-ui/src/components/ConsentBanner/useConsent.ts` (extend) + dossier banner spec
- C đụng `kiteclass/kiteclass-core/src/main/java/com/kiteclass/core/module/{course,invoice,payment}/` + Flyway `V67__add_pricing_model_to_courses.sql` (+ paired `R67__undo`) + `kiteclass-frontend/src/app/(dashboard)/courses/` FE radio form + business docs `BR-COURSE-PRICING-001..004` + ADR-027
- D đụng `kiteclass-core/src/main/java/com/kiteclass/core/module/clazz/{service/ClassServiceImpl.java,controller/ClassController.java}` + Flyway `V68__add_class_reschedule_audit.sql` (audit log only, NO enum change) + `kiteclass-frontend/src/app/(dashboard)/classes/[id]/` FE modal + email template `kitehub/kitehub-email/src/main/resources/templates/class-rescheduled.html.mustache` + ADR-030
...
- C vs D: cả 2 trong `kiteclass-core` nhưng khác module — C `course/invoice/payment`; D `clazz`. Maven compile parallel OK. ⚠️ Migration ordering: C ships V67, D ships V68 (sequential reservation per §3)
...
- Cross-bucket business semantic (Agent 3 Cell 5): reschedule mutates session date affecting invoice period — lock semantic `BR-COURSE-PRICING-004 + BR-CLASS-RESCHEDULE-002` per §3
...
**Cross-layer? (per `wave-pack-planner/SKILL.md` §Step 4.5):** YES (E cross-cut Mustache helper consumed by B + D email templates). Skip Bucket 0 Foundation but lock cross-bucket coordination table §3.5 below.
...
| B | **PDPL consent API + hash chain + lifecycle handler** | GAP-353b | P1 | `kitehub/kitehub-subscription/src/main/java/.../consent/` (new), `kitehub-subscription/src/main/resources/db/migration/V26__create_consent_record.sql`, `packages/shared-ui/src/components/ConsentBanner/useConsent.ts`, dossier banner spec | parallel after A |
| C | **Pricing PER_HOUR + GAP-292b payment recording paired** | GAP-292 + GAP-292b | P0 | `kiteclass/kiteclass-core/src/main/java/com/kiteclass/core/module/{course,invoice,payment}/`, `V67__add_pricing_model_to_courses.sql` (+ `R67__undo`), `kiteclass-frontend/src/app/(dashboard)/courses/`, business docs `BR-COURSE-PRICING-001..004`, ADR-027 | parallel after A |
| D | **Reschedule + email fallback + reason MANDATORY** | GAP-291 | P0 | `kiteclass-core/.../module/clazz/`, `V68__add_class_reschedule_audit.sql` (audit log only, NO enum), `kiteclass-frontend/src/app/(dashboard)/classes/[id]/`, `kitehub-email/src/main/resources/templates/class-rescheduled.html.mustache`, ADR-030 | parallel after A |
...
- Files: `infrastructure/terraform-aws/secrets.tf` (add `random_password.resend_api_key_placeholder` + `aws_secretsmanager_secret.resend_api_key` resource + `aws_secretsmanager_secret_version` với lifecycle ignore_changes pattern matching jwt-challenge-secret precedent Wave 81 GAP-509), `infrastructure/terraform-aws/iam.tf` (verify wildcard `kitehub/production/*` pattern covers — NO explicit grant edit needed), `scripts/fetch-secrets.sh` (add line `RESEND_API_KEY=$(fetch_secret resend-api-key)` + add to `/etc/kite/.env` template), `scripts/audit-env-coverage.sh` (extend ACCEPTABLE_DEFAULTS với well-commented per-row rationale; ship unit tests với fixtures known good + known bad config), `.github/workflows/script-quality.yml` (add job `env-coverage` WARN-mode initially per `incident-to-rule-pipeline.md` §3.1 tightened defer conditions), `documents/02-architecture/env-vars-registry.md` (add RESEND_API_KEY row + Wave br-3 IaC parity note)
- Tests: `scripts/test/audit-env-coverage-test.sh` (3 fixture cases: known good + known false positive + known missing)
- Acceptance:
...
- Files: `kitehub/kitehub-subscription/src/main/java/com/kitehub/subscription/consent/` (new package — `ConsentRecord.java` entity + `ConsentService.java` + `ConsentController.java` + `ConsentRepository.java`), `kitehub-subscription/src/main/resources/db/migration/V26__create_consent_record.sql` (immutable schema: `consent_record(id BIGSERIAL, user_id BIGINT NULL, tenant_id BIGINT NULL, granted JSONB NOT NULL, prev_hash VARCHAR(64) NULL, current_hash VARCHAR(64) NOT NULL, ip_address INET NOT NULL, user_agent TEXT NOT NULL, signed_at TIMESTAMPTZ NOT NULL DEFAULT NOW())` + RLS policy NO UPDATE NO DELETE), `packages/shared-ui/src/components/ConsentBanner/useConsent.ts` (extend với `POST /api/v1/consent/record` call + analytics SDK lifecycle handler — `gtag('consent', 'update', {analytics_storage: 'denied'})` synchronously BEFORE server API call on revoke), `documents/02-architecture/dossier/14-common-components-inventory-kh.md` (add ConsentBanner spec row), `documents/04-quality/compliance/pdpl-pre-launch-checklist.md` (new — "v1 pending counsel review" disclaimer per CLAUDE.md Phase 1 BETA risk tolerance), ADR-029 (cookie consent vendor: self-build banner + iubenda privacy policy generator ~$27/year)
- Tests: `kitehub-subscription/src/test/java/.../consent/ConsentControllerIT.java` (POST/GET endpoints + hash chain integrity), `ConsentBannerLifecycleTest.test.tsx` (RTL — verify analytics SDK denied call before server POST trên revoke)
- Acceptance:
  - [ ] Migration V26 immutable schema (NO UPDATE NO DELETE per RLS) + hash chain `prev_hash → current_hash` (SHA-256)
  - [ ] `POST /api/v1/consent/record` accepts granular toggles JSON `{necessary: true, analytics: bool, marketing: bool, preferences: bool}` + computes current_hash từ row content + persists actor signing (user_id soft reference NO FK, ip_address, user_agent, signed_at server time)
  ...
  - [ ] IT test `concurrent_audit_log_writes_preserve_hash_chain` PASS (per Agent 3 Cell 8 shared `AdminAuditLogService.log()` with `@Transactional(isolation = SERIALIZABLE)`)
  ...
  - [ ] Status target = 🟡 PARTIAL (counsel review deferred per Phase 1 BETA risk tolerance; PDPL deadline 2026-07-01 per CLAUDE.md)
...
- Files: `kiteclass/kiteclass-core/src/main/java/com/kiteclass/core/module/course/entity/Course.java` (add `@Enumerated(EnumType.STRING) @Column(name="pricing_model") private PricingModel pricingModel; @Column(name="unit_price") private BigDecimal unitPrice;` — DEPRECATE `price` field với `@Deprecated` annotation), new `module/course/entity/PricingModel.java` enum (PER_HOUR, MONTHLY, COURSE_PACKAGE, FREE per user decision Q2), new `module/payment/entity/PaymentMethod.java` enum (CASH, BANK_TRANSFER, VIETQR, MOMO per user decision Q3), new `module/payment/entity/PaymentRecord.java` entity (`invoice_id FK, amount_paid, payment_method, paid_at, recorded_by_user_id, notes`), new `module/payment/controller/PaymentRecordController.java` (`POST /api/v1/invoices/{id}/record-payment`), update `module/invoice/service/InvoiceGenerationService.java` (per-model logic: PER_HOUR = `sum(attended_sessions × session_hours × unit_price)` trong period; MONTHLY = `unit_price × N_months`; COURSE_PACKAGE = `unit_price` 1-shot; FREE = 0), Flyway `V67__add_pricing_model_to_courses.sql` (NOT NULL với default `COURSE_PACKAGE` cho existing rows) + paired `R67__undo_pricing_model.sql` (rollback script), `V67b__create_payment_records.sql`, `kiteclass-frontend/src/app/(dashboard)/courses/[id]/edit/page.tsx` (radio form "Hình thức tính học phí: ⚪ Theo giờ (vd 200.000đ/giờ) ⚪ Theo tháng (vd 1.500.000đ/tháng) ⚪ Trọn gói khoá (vd 8.000.000đ/khoá) ⚪ Miễn phí"), `kiteclass-frontend/src/app/(dashboard)/invoices/[id]/page.tsx` (button "Đánh dấu đã thu" + modal radio PaymentMethod), business docs `documents/01-business/kiteclass/course/rules.md` BR-COURSE-PRICING-001..003 (5-attribute per `business-logic-review.md`) + `BR-COURSE-PRICING-004` cross-bucket reschedule semantic, `documents/01-business/kiteclass/payment/rules.md` BR-PAYMENT-METHOD-001..002, ADR-027 pricing taxonomy (PER_HOUR primary justified by Apollo 257-344k/giờ ILA 195-368k/giờ VN TT Anh ngữ benchmark)
- Tests: `kiteclass-core/src/test/java/.../course/PricingModelTest.java` (4 enum × invoice scenarios), `payment/PaymentRecordControllerIT.java` (record-payment endpoint + duplicate prevention via Idempotency-Key Wave br-2 pattern), cross-bucket IT `kiteclass-core/src/test/java/.../cross/RescheduleInvoicePeriodSemanticsIT.java` (Agent 3 Cell 5 — reschedule session #5 mutates date → invoice period recalc khi cross boundary)
- Acceptance:
...
- Acceptance:
  ...
  - [ ] ClassRescheduledEvent published via Outbox pattern (no direct fast-path — Wave 91 outbox dispatcher consumes)
...
- Tests: `kitehub-email/src/test/java/.../template/PersonaToneResolverTest.java` (5 persona × greeting matrix), `scripts/test/vn-sample-fixtures-test.sh` (fixture cases: known good VN sample + known bad English placeholder)
- Acceptance:
...
### 3.5 Cross-bucket lock decisions (mandatory BEFORE agent spawn)
...
8. **Email tone matrix mandate:** Bucket B (consent emails if any) + Bucket D (class-rescheduled email) MUST consume `_shared/persona-tone.mustache` partial — Bucket E ships partial; B/D ship template consumers (compile-time integration check post-merge)
9. **Cross-bucket IT tests mandatory:**
   - `consent_revoke_invalidates_analytics_within_5s` (Bucket B — Agent 3 Cell 6)
   - `reschedule_preserves_invoice_period_semantics` (Bucket C + D cross — Agent 3 Cell 5)
   - `concurrent_audit_log_writes_preserve_hash_chain` (Bucket B — Agent 3 Cell 8 shared `AdminAuditLogService.log()` `@Transactional(SERIALIZABLE)`)

---

## 4. Sequencing (Gantt simplified)

```
T+0     Bucket A spawn (Agent A Opus, ~2.5h)
T+0.5h  Bucket A CI ready (script tests + WARN mode) — code merge OK trước AWS verify
T+0.5h  Bucket B + C + D parallel spawn (Agents B/C/D Sonnet, ~3.5h cap)
T+4h    Bucket B + C + D merge OK
T+4h    Bucket E spawn (Agent E Sonnet, ~1.5h) — consume B/D email templates
T+5.5h  All 5 buckets merged
T+6h    Coordinator: wave-history.jsonl append + ROADMAP §🎯 update + GAP CSV sync + memory mirror per `post-merge-sync-completeness.md` §2 4-target
T+6.5h  Wave plan flip status: draft → complete
```

**Total wall-clock:** ~6.5h (compressed from serial ~13h via parallel execution per `feedback_parallel_agent_strategy.md`).

---

## 5. Closure Protocol (per `wave-closure-scope-completeness.md` §3)

Before flip status: draft → complete:

1. **Scope-Completeness Reconciliation table** — verify all 5 buckets AC checked + 4 follow-up gaps filed nếu PARTIAL
2. **Post-wave audit suite trigger** (per `post-wave-audit-mandate.md` 3-day deadline) — schedule Security + Business-Logic + API contract + Ops Readiness audits within 3 ngày của closure
3. **4-target sync** per `post-merge-sync-completeness.md` §2:
   ...
   - `ROADMAP.md §🎯 Current Status Snapshot`: Wave br-3 SHIPPED entry
   - `wave-history.jsonl`: Wave br-3 outcome entry (5 buckets shipped, follow-ups filed, audit suite scheduled)
   - `MEMORY.md`: new memory entry `feedback_outside_in_audit_value_wave_br_3.md` (3-agent audit prevented 7 P0 + 10 P1 cells)
4. **GAP-292 / GAP-291 / GAP-508 / GAP-353b flip per `gap-done-discipline.md` §2:** AC checked + no banned phrases + follow-up filed for any PARTIAL deferral
5. **Session-handoff note** per `session-end-context-check.md` §4.5 row 5: `documents/03-planning/session-handoffs/2026-05-24-wave-br-3-closure.md`
...
|---|---|---|---|---|
| 1 | Bucket A AWS verify blocked GAP-612 → silent IaC drift if Resend account verification fails post-restore | P0 | PARTIAL status target ~90% + paired follow-up gap + Resend account creation pre-arranged parallel với GAP-612 escalation | Agent 3 Cell 7 |
| 2 | Migration V67 (Bucket C) race với V68 (Bucket D) khi 2 agents parallel claim version | P0 | Hardcode reservation §3.5; cross-bucket merge dry-run script `git merge-tree main wave-c wave-d \| grep "V67"` pre-merge | Agent 3 Cell 1, Wave 105 precedent |
| 3 | Bucket B consent_record cross-database FK → migration fails OR silent skip | P0 | Lock soft reference no FK §3.5; document inline migration comment | Agent 3 Cell 4 |
| 4 | Bucket C existing tenant data default COURSE_PACKAGE wrong for MONTHLY-billed courses → invoice generation bug | P1 | Pre-migration audit script `scripts/audit-pre-pricing-model.sql` + email Phase 1 BETA Owners verification | Agent 3 Cell 12 |
| 5 | Bucket D ClassRescheduledEvent outbox without consumer → unbounded queue grow | P1 | Default no-op consumer + feature flag `notify.enabled=false` default; outbox queue depth alert | Agent 3 Cell 10 |
| 6 | Cross-bucket reschedule + invoice generation interaction → wrong invoice amount | P0 | Cross-bucket IT test `reschedule_preserves_invoice_period_semantics` mandatory; BR-COURSE-PRICING-004 + BR-CLASS-RESCHEDULE-002 document semantic | Agent 3 Cell 5 |
| 7 | Bucket B analytics SDK lifecycle handler lag on revoke → PDPL Art 13 audit fail | P0 | Synchronous `gtag('consent', 'update', {analytics_storage: 'denied'})` BEFORE server API call; IT test verify ≤5s | Agent 3 Cell 6 |
| 8 | Bucket B PDPL pre-launch checklist solo-dev no counsel → banned-phrase risk on DONE flip | P1 | PARTIAL exit ramp per `gap-done-discipline.md` §3; "v1 pending counsel review" disclaimer per CLAUDE.md Phase 1 BETA risk tolerance | Agent 3 Cell 11 |
| 9 | VN sample data drift trong test fixtures + email previews → English placeholder leak to user | P1 | Bucket E `scripts/audit-vn-sample-fixtures.sh` + reviewer-checklist mandatory per `vn-localization-audit-checklist.md` §2 row 7 | Persona Agent 1 Pattern 2 |
| 10 | Persona tone mismatch trong 3 email templates → Vy formal greeting "Kính gửi quý" defeats target | P1 | Bucket E shared Mustache helper `_shared/persona-tone.mustache` consumed by B/D | Persona Agent 1 Pattern 1 |

---

## 7. Post-wave follow-ups (filed at closure)

| Gap | Priority | Scope | Trigger |
|---|---|---|---|
| GAP-NEW-resend-live-verify-post-restore | P1 | Live verify Resend API key delivery + DKIM/SPF/DMARC + smoke test 5 VN ISPs (Gmail/VietTel/VNPT/VNG/Yahoo) | GAP-612 AWS restore unblock |
...
| GAP-NEW-zalo-oa-notification-integration | P2 | GAP-063 Phase 2 Zalo OA notification consumer subscribes ClassRescheduledEvent | Wave br-4+ |
| GAP-NEW-vat-einvoice-misa-integration | P2 | MISA MeInvoice partnership investigation per Agent 2 F5 + Wave 93 GAP-185 precedent | Wave br-4+ |
...
## 8. Outside-in audit artifacts (3 agents)

3 agents ran 2026-05-24 parallel ~5 phút each:
- **Persona simulation** (4 personas × 4 gaps = 16 cells): 3 P0 + 5 P1 + 3 P2 + 3 cross-cut patterns
- **VN edu SaaS benchmark** (6 VN SaaS + 4 international): 4 P0 + 4 P1 + 4 ADR recommendations + 5 vendor managed > self-build matrix
- **Failure-mode matrix** (10 classes × 8 triggers × 6 personas = 480 cells, picked 22): 7 P0 + 10 P1 + 5 P2 + 3 cross-cut patterns

Synthesized findings folded into §3 Bucket scope + §3.5 cross-bucket lock decisions + §6 risk register + §7 follow-up gaps. **Counterfactual without outside-in:** Wave br-3 would have shipped GAP-292 PER_SESSION wrong taxonomy + GAP-292 missing payment recording + GAP-291 no email fallback + GAP-353b no hash chain + Bucket A IaC drift + Migration V67 race — minimum 4-5 retroactive fix waves + production incidents.

---

## 9. Log

- **2026-05-24:** Wave plan drafted. 3 outside-in agents ran parallel (persona simulation + VN edu benchmark + failure-mode matrix). User 3-Q decisions chốt: (a) PDPL deadline giữ CLAUDE.md 2026-07-01, (b) Pricing taxonomy = PER_HOUR primary, (c) Bucket C ghép GAP-292b paired payment recording. Wave plan ship via PR per `feedback_wave_plan_through_pr.md` BEFORE agent spawn implementation.
