...
- **2026-06-06 (Teacher provisioning shipped — Hướng B):** Admin-driven teacher credential via new `POST /api/v1/teachers/{id}/credentials` (`@PreAuthorize hasAnyRole OWNER/ADMIN/PRINCIPAL`) → `TeacherService.provisionCredential` → `AuthCredentialProvisioningService.setPassword` (UPSERT — admin set/reset, vs idempotent `provision` for self-redeem). New `auth/dto/SetPasswordRequest` (reusable, 8-100 + letter/digit/symbol). Avoided touching `CreateTeacherRequest` record (~10 test callers) — separate provisioning endpoint instead. **Walk PASS:** admin set credential 200 → TEACHER credential created (entity_id=1) → teacher login role=TEACHER referenceId=1 → non-admin set 403 → weak password 400. TeacherServiceTest 9/9 PASS. GAP-725 teacher path = DONE. Remaining: student provisioning + KC-9 (Bucket E), production parity.
- **2026-06-06 (Bucket B shipped + real-provisioning walk PASS):** Wired parent credential provisioning into `ParentInvitationServiceImpl.redeem` via new `AuthCredentialProvisioningService` (auth module, idempotent-on-email, atomic in redeem txn — not audit scope so default propagation). Redeem already had `password` field + `invitation.email` as login email (designed for this, never wired per GAP-798b). Enabled `PARENT_PORTAL_ENABLED=true` (was Wave 2 default-off gate) — redeem now provisions credential. **Removed V89 dev seed** (production hygiene — no known-password row ships; redeem provisions at runtime).
  **Bucket B walk (real provisioning, NO seed):** seed PENDING invitation (parent-walk@test.com → student 2) → redeem 200 (parentId=3 + credential auto-created entity_id=3) → login new parent through gateway :9000 → 200 JWT → `/parent/me/children` 200 [child 2] → isolation read child 1 → 403. **Full chain invite→redeem→credential→login→gateway→facet verified end-to-end.**
  PARENT auth COMPLETE (login + provisioning). GAP-725 parent path + GAP-798b parent reference_id producer = DONE. Remaining: teacher provisioning (TeacherController→credential), student provisioning + KC-9 (Bucket E), production parity (kiteclass-core prod JWT_SECRET + PARENT_PORTAL_ENABLED via fetch-secrets/deploy).
