...
- **2026-06-14 (wave-2026-06-14-p0-closeout-1 Bucket A — branch `fix/wave-p0-a-lms-paywall`):** Re-verified fix vẫn còn trên main `cd89a3ddc` + **hardening**. Thay vì `LessonProgressServiceImpl` tự inject `EnrollmentRepository`+`ClassRepository` và dup logic enrollment-check (mirror của `LmsServiceImpl`), giờ inject component dùng chung `LessonAccessGuard.verifyStudentEnrollment(...)` — write-path (gap này) + read-path (GAP-1115) dùng CHUNG đúng 1 guard, loại bỏ rủi ro drift giữa "đọc nội dung paid" và "complete bài paid". Cross-flow sweep (xem GAP-1115 Log) xác nhận `completeLesson` là write-path duy nhất ghi progress bài paid (FIX). `LessonProgressServiceTest` 3-case (deny-paid-not-enrolled + allow-paid-enrolled + allow-trial-no-enrollment) cập nhật mock guard, PASS. Vẫn 🟡 PARTIAL — AC 5 (human runtime-walk gateway :9000) chưa thực hiện được bởi agent → giữ PARTIAL trước DONE flip.
