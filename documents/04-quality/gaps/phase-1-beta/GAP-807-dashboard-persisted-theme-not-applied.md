...
- [ ] `(dashboard)/layout.tsx` fetch + apply persisted tenant branding CSS vars on mount
- [ ] Sau update branding → dashboard re-apply không cần hard reload
- [ ] Test cover persisted-apply path (fetch → apply CSS vars)
- [ ] Live-walk: owner đổi màu → save → reload dashboard → thấy màu custom (per `feature-ship-runtime-walk-mandate.md`)
