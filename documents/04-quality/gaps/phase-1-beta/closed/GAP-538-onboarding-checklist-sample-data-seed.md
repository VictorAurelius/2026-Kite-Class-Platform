...
- 2026-05-21 — Wave 102.9 Bucket B state-check: ALL implementation AC verified shipped per `documents/04-quality/audits/persona-review/2026-05-21-wave-102.9-bucket-b-tenant-init-onboarding-state-check.md`. Grep confirms ZERO Lorem Ipsum/John Doe/Class A1/Example Center in `kitehub-frontend/src/components/onboarding-checklist/` + `src/app/(customer)/onboarding/`. VN labels (`Bắt đầu với KiteHub`, `Hoàn tất hồ sơ tenant`, `Nhập dữ liệu mẫu`) verified via Playwright e2e spec assertions. Day-1 seed canonical path = `kitehub/scripts/seed-data.sh` + `VietnamSampleDataGenerator` Spring @Component (task brief reference `seed-day1-data.sh` was wishful — canonical name is `seed-data.sh`). Live walkthrough verify still blocked GAP-612 AWS account suspended; no new follow-up gap needed per `pre-handoff-self-test-completeness.md` §5.4 (pre-existing blocker dependency).
...
- 2026-06-01 — Wave onboarding-polish-2 Bucket D follow-on polish (gap đã DONE 100% từ Wave 102.9; không reopen): state-check phát hiện `kitehub/scripts/seed-data.sh` courses array vẫn dùng English placeholders (`English Basics` / `IELTS Preparation` / `Business English` / `Kids English`) → drift so với AC8 "course names tiếng Việt". Wave plan originally targeted `ProductionSeedRunner` nhưng state-check confirm `seed-data.sh` IS canonical dev seed path (`ProductionSeedRunner` chỉ seed admin user + system_config rows). Translated 4 English courses → 5 VN K-12 + ngoại ngữ mix (`Toán 6` / `Văn 7` / `Tiếng Anh Cambridge` / `Vật lý 10` / `Tin học cơ bản`) per `.claude/rules/vn-localization-audit-checklist.md` §2 row 3 + cover K-12 + language center diversity. Added `scripts/tests/test-seed-data-vn-content.sh` (4-assertion bash test: ≥5 VN diacritic name markers + ≥3 VN course tokens + 0 banned English placeholders + ≥2 VN teacher names — all PASS). Per `gap-done-discipline.md` "🟢 DONE never re-opens" — gap stays DONE 100%, this entry log continuity polish only. No CSV row status change.
