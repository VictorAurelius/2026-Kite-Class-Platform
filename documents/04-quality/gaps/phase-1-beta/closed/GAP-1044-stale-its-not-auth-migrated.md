...
- [x] CI test strategy for `*IT` decided + documented (failsafe wire OR rename OR opt-in)
- [x] 3 ITs auth-migrated + SUB-20-aligned, pass when run
- [x] `SubscriptionBillingIT` context loads (or is intentionally excluded with rationale)
...
🟢 DONE — engineering-complete + G3 production-parity walk verified. Test infra — stale *IT auth-migrated (commit bbba1581f).
