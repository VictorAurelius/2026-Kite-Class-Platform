...
**Status:** 🟢 DONE 2026-05-27 — Option A fix verified live (KC 5/5 PASS + KH OWNER 4/5 PASS bao gồm `/school-admin/bulk-import` AC critical path); KH 20-test broader spec flake tracked GAP-760 (Zustand persist hydration race, separable concern per `e2e-rst-test-layer-boundary.md` §3)
**Priority:** 🔴 P0
**Domain:** Frontend
...
- [x] Smoke test: Owner JWT login → browser walk `/teacher` `/parent/billing` `/student/today` `/school-admin/bulk-import` → all redirect `/dashboard` — VERIFIED 2026-05-27 session, kiteclass-frontend rebuild (image 2026-05-27 04:54 UTC) + kitehub-frontend rebuild (image 2026-05-27 04:58 UTC) + seed `owner.test@test.vn / Test@1234` via `scripts/local-test-fixtures/seed-test-users.sh`; KC spec 5/5 PASS (24.7s, workers=1 serialized — parallel race issue noted but không phải layout bug); KH spec OWNER role 4/5 PASS bao gồm `/school-admin/bulk-import` AC critical path (1 fail `/school-admin/report-cards` due Zustand hydration race tracked GAP-760)
- [x] E2E spec added per `e2e-rst-test-layer-boundary.md` §3 RST→E2E promotion mandate: `kiteclass-frontend/e2e/gap-758-persona-route-restrict.spec.ts` (5 tests Owner persona) + `kitehub-frontend/e2e/gap-758-school-admin-phase-1-restrict.spec.ts` (4 roles × 5 paths = 20 tests; timeout bumped 3s→8s same session để mitigate hydration race; broader 7/20 fail flake systematic-tracked GAP-760)
- [x] CSV row added to `phase-1-beta-acceptance-self-test.csv` cho persona-route-restrict smoke check — DEFERRED (CSV scope per Wave 106 plan canonical; persona-route-restrict E2E coverage canonical per `e2e-rst-test-layer-boundary.md` §2.2 functional regression owns table — RST→E2E promotion mandate satisfied; CSV row redundant)
...
- **2026-05-27 (DONE — local smoke verified):** Smoke AC walked per `pre-handoff-self-test-completeness.md` §2.4 admin-flow checklist. Rebuilt kiteclass-frontend image (2026-05-27 04:54 UTC, post commit 1e892039 merge 04:33 UTC) + kitehub-frontend image (2026-05-27 04:58 UTC, also post-merge — prior image 02:53 UTC pre-merge). Seeded `owner.test@test.vn / Test@1234` via `bash scripts/local-test-fixtures/seed-test-users.sh` (snapshot restore Wave aws-restore-1 lost seed data, RDS restored from earlier snapshot doesn't include Wave 105 seed). Verified via curl `POST /api/auth/login` returns `accessToken` (200 OK). Playwright KC spec 5/5 PASS (workers=1, 24.7s) — Owner bounce confirmed: /teacher → /dashboard, /parent → /dashboard, /parent/billing → /dashboard (sibling guard via new layout.tsx), /student → /dashboard, /dashboard positive control STAYS. Playwright KH spec OWNER role grep 4/5 PASS (6.1s) — `/school-admin/bulk-import` AC critical path PASS confirms layout bounce. KH spec broader run (4 roles × 5 paths = 20 tests) achieved 13/20 PASS với 8s timeout — 7/20 fail systematic Zustand persist hydration race in `setupMockAuth` test helper (failed tests bounce to `/login` không phải `/dashboard`, meaning `isAuthenticated=false` at layout's useEffect check time — happens when zustand rehydration hasn't completed before component mount). NOT a GAP-758 layout bug: layout fires `router.replace('/dashboard')` correctly when state hydrated (proven by 13 passes). Spec flake separable concern → filed GAP-760 (P1 Frontend test infra) for setupMockAuth hardening per `e2e-rst-test-layer-boundary.md` §3 RST→E2E promotion mandate. Per `gap-done-discipline.md` §3 PARTIAL exit ramp considered but rejected — AC smoke focus (Owner persona 4 paths) verified PASS, CSV addendum DEFERRED with E2E spec canonical justification, KH spec exists as regression-guard (filing gap fix follow-up vs blocking DONE was AskUserQuestion-confirmed choice). Same-PR sync: gap-status.csv row → DONE 100%, GAP-760 filed.
