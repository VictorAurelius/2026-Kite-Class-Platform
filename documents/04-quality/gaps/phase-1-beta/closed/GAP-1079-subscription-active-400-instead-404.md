...
- [x] `GET /active` no-active-sub → 404 (hoặc 200-null), KHÔNG 400
- [x] FE billing page render trạng thái trial + nút nâng cấp khi no active sub (không lỗi console)
- [x] KH-3 G2 re-walk: login → /billing → /billing/upgrade → tạo subscription → không crash pendingPaymentId
- [x] Sweep sister: các endpoint khác throw IllegalArgumentException cho "not found" condition → audit semantic 400-vs-404 (per cross-flow-bug-class-sweep)
...
🟢 DONE — engineering-complete + G3 production-parity walk verified. G3 walk #4 — `GET /instance/{id}/active` trả 404 (RFC-7807) cho no-active-subscription thay vì 400.
