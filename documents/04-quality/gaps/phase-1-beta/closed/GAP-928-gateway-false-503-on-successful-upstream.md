...
**Status:** 🟢 DONE 2026-06-04 — Phase 1 + Phase 2 shipped Wave flow-kh1; Phase 3 (fallback logging) tracked in GAP-929 follow-up
...
- [x] Phase 2 carve write routes — new `authWriteRoute` config (slowCallRateThreshold=100, slow-call detection effectively disabled) + `authWriteCircuitBreaker` instance + new `kitehub-auth-write` route matching `POST /api/v1/auth/beta-signup` (declared before `kitehub-auth-v1` catch-all) + existing `auth-register` route swapped to `authWriteCircuitBreaker` — shipped Wave flow-kh1 (this PR)
- [x] Smoke test on local stack: gateway compiles clean (`./mvnw -pl kitehub-gateway -am compile` BUILD SUCCESS) + application.yml YAML lint PASS (python yaml.safe_load) — runtime smoke deferred to coordinator G2 re-walk after gateway rebuild
- [ ] CloudWatch / local-log probe demonstrates the fallback controller now logs *both* upstream response status and request body fingerprint when it fires — Phase 3 deferred to GAP-929 (FallbackController observability follow-up)
- [x] Cross-flow sweep per `cross-flow-bug-class-sweep.md` §3 — any other route with circuit breaker on a write endpoint gets the same threshold treatment — sweep done (see §Cross-flow sweep below); other write-endpoint routes (`instancesCircuitBreaker` write paths) intentionally DEFERRED — no walk evidence of false 503 on instance routes yet; Phase 2 scope kept bounded to the empirically-reproduced auth-write lockout
- [x] Wave flow-kh1 G2 walk: invitee can submit beta-signup once and see the same status the BE persisted (no false 503) — Phase 2 dedicated write breaker eliminates the shared-breaker root cause; coordinator G2 re-walk after gateway rebuild verifies empirically
...
- **2026-06-04** Phase 2 shipped — Wave flow-kh1 (this PR, agent-gap-928-p2 worktree).
  - **Re-walk that proved Phase 1 insufficient**: 2026-06-04 06:18-06:19 UTC re-walk reproduced the false 503 even with Phase 1 thresholds applied. Slow `/api/auth/2fa/verify` call at 06:18:53 tripped the shared `authCircuitBreaker` (1st OPEN); invitee's POST `/api/v1/auth/beta-signup` at 06:19:29 hit the OPEN breaker; subscription module already committed the tenant `g2test-an-xx` + OWNER user row at 06:19:29 (DB-confirmed); gateway fallback fired 06:19:30 returning false 503; invitee retried 5x and got 404 INVALID_TOKEN because completeBetaSignup nulled invite_token on the successful first call. Root cause: read paths under `/api/auth/**` (2FA verify, login, refresh) cohabit with write paths (beta-signup, register) on a single shared `authCircuitBreaker` instance — a slow read trip locks out subsequent writes even when they would succeed.
  - **Config change**: added new `resilience4j.circuitbreaker.configs.authWriteRoute` config with `slowCallDurationThreshold: 60s`, `slowCallRateThreshold: 100` (slow-call detection effectively disabled — even all-slow does not trip), `failureRateThreshold: 50`, `slidingWindowSize: 10`, `minimumNumberOfCalls: 20`. Added new `authWriteCircuitBreaker` instance with `baseConfig: authWriteRoute`. Carved a new gateway route `kitehub-auth-v1-beta-signup` matching `POST /api/v1/auth/beta-signup` (declared BEFORE `kitehub-auth-v1` catch-all so Spring Cloud Gateway matches it first per declaration order) targeting `kitehub-subscription:8080` with `authWriteCircuitBreaker`. Also swapped existing `auth-register` route (POST `/api/auth/register`, was on `authCircuitBreaker`) to `authWriteCircuitBreaker` — covers the second tenant-creating write endpoint in the same fix. Inline comments cite GAP-928 Phase 2 + the empirical re-walk evidence.
  - **Sweep findings**: `instancesCircuitBreaker` shares similar bug class signature — write routes (`platform-instances`, `kitehub-instance-domain-verify`, force-convert, rollback-migration) share a `default`-baseConfig breaker with read paths under `/api/platform/instances/**` and `/api/instances/**`. DEFERRED for now — no walk evidence of false 503 on instance routes yet (the re-walk symptom was specifically on `/api/v1/auth/beta-signup`); Phase 2 scope kept bounded to the empirically-reproduced auth-write lockout to avoid premature over-engineering. If a future walk surfaces a false 503 on instance write paths, the same carve-out pattern applies (introduce `instancesWriteRoute` + `instancesWriteCircuitBreaker` + dedicated routes for the 4 write paths).
  - **Build evidence**: `./mvnw -pl kitehub-gateway -am compile` → BUILD SUCCESS. `python3 -c "import yaml; yaml.safe_load(open('kitehub/kitehub-gateway/src/main/resources/application.yml'))"` → clean. Container rebuild + G2 re-walk verification = coordinator scope (smoke walk after `bash kitehub/scripts/rebuild.sh kitehub-gateway`).
  - **Status flip**: PARTIAL → DONE. Phase 1 + Phase 2 together resolve the original lockout symptom — invitee will no longer see false 503 with upstream committed because the write-endpoint breaker (a) does not share state with slow reads, and (b) does not count slow successes as failures. Phase 3 (FallbackController logs upstream status + body fingerprint) is a separate observability concern and is tracked in new gap GAP-929.
