...
**Status:** 🟢 DONE (Wave 98 Bucket B5 — SupportMenu canonical entry point shipped; paid chat widget vendor out-of-scope)
**Priority:** 🔴 P0
**Domain:** Frontend
**Detected:** 2026-05-14
...
- 2026-05-18 — Wave 98 Bucket B0 GAP-656 ship `SupportMenu.tsx` consolidating support + feedback floating button into single `?` dropdown bottom-right (eliminates GAP-540 + GAP-542 mobile ≤375px collision per outside-in audit F-NEW-2/4). SupportMenu provides 4-item dropdown: Hướng dẫn nhanh / Liên hệ hỗ trợ (mailto:support@kitehub.me) / Gửi phản hồi (placeholder, B5 wires actual FeedbackForm modal) / Trạng thái beta. Standalone chat widget vendor still deferred — SupportMenu is canonical entry point từ Wave 98 forward.
- 2026-05-18 — Wave 98 Bucket B5 — Status flip to 🟢 DONE. SupportMenu "Gửi phản hồi" placeholder modal replaced với actual `FeedbackForm` Radix Dialog (`kitehub-frontend/src/components/feedback/FeedbackForm.tsx`). All AC met: (a) Footer support@/Help/beta-status shipped Wave 78; (b) SupportMenu canonical in-app entry point shipped Wave 98 B0; (c) `?` dropdown nav visible across dashboard layout via `OnboardingCoordinator`; (d) Vietnamese narrative throughout per `dev-readable-doc-language.md`; (e) accessibility = Radix DropdownMenu + Dialog focus-trap + Escape + WCAG 2.5.5 touch target ≥44×44px; (f) Playwright mobile spec extended với FeedbackForm flow regression. **Paid chat widget vendor (Crisp/Tawk.to) explicitly out-of-scope** — Tier 1 mailto:support@kitehub.me + `?` dropdown discoverability satisfies Phase 1 BETA scope; vendor evaluation defers to Wave 79+ per outside-in audit. CSV row: `phase-1-beta/closed/GAP-540-*.md`, status DONE, completion 100. File `git mv` to closed/ per `gap-folder-organization.md` v2.0.0 §3.3.
- 2026-05-18 — Wave 98 Bucket B6 GAP-660 paired closure: Zalo OA fast-path active replacing previously-deferred "Phase 1.5 coming" copy. SupportMenu now has 5-item dropdown (added "Liên hệ Zalo OA" between email and feedback) with mobile deep-link `zalo://chat?oa_id={oa_id}` + desktop web fallback. Footer adds inline Zalo OA link in Support column. Beta-invite + invite-staff email templates (html + txt) include Zalo OA CTA. Env var `NEXT_PUBLIC_KITEHUB_ZALO_OA_ID` with placeholder fallback `kitehub`. AC4 Zalo OA Phase 1 BETA now satisfied via GAP-660 (sister gap closed).

- **2026-05-18 (PR #1555 + #1557 merged)** — Sync per `post-merge-sync-completeness.md` §4. PR #1555 (B5) closed FeedbackForm modal scope; PR #1557 (B6 sister) closed Zalo OA AC4 — full GAP-540 AC matrix verified.
