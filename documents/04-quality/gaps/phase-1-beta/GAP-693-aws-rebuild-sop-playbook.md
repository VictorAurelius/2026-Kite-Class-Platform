...
**Status:** 🔵 OPEN (BLOCKED — depends on GAP-612 resolve + GAP-691 DONE + GAP-692 Phase 1 DONE)
**Priority:** 🔴 P0 (META — single SOP eliminates 8 prior failure-mode classes)
**Domain:** DevOps + Meta
**Detected:** 2026-05-21
**Related PRs:** TBD
**Related Docs:** Outside-in audit synthesis 2026-05-21; GAP-612; GAP-691; GAP-692
...
User-chosen sequencing 2026-05-21: Phase 0 local self-test (GAP-691) → Item 2 refactor (GAP-692) → rebuild. This gap = step 3 execution playbook. **CANNOT execute until:**
- GAP-612 resolved (account #1 fate determined OR account #2 LEGIT path chosen)
- GAP-691 DONE (local smoke working — required by Gate 3 in this SOP)
...
**§2 — Pre-flight checklist** (account ready + state backend + GAP-612 resolved + GAP-691 + GAP-692 Phase 1 done)
...
- Gate 2 (before Step 8): `scripts/local/preflight-stack.sh` PASS — local stack healthy (per GAP-691)
- Gate 3 (before Step 9): `scripts/local/smoke-e2e.sh` PASS — Playwright headless P2+P3+admin flows green (per GAP-691)
...
- **GAP-691** local self-test investigation (prerequisite — Gate 2/3 require working local stack)
