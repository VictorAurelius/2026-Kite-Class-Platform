...
- **2026-05-28 (path-scope fix, same PR #1958):** User flagged "GAP-802 đã check cho tùy biến UI chưa?" → sweep phát hiện detector #2 + rule #4 trỏ sai `kitehub/kiteclass-frontend` (KiteClass FE thật ở `kiteclass/kiteclass-frontend`) → #2 không scan KiteClass FE, `/reset-password` là FALSE POSITIVE. Fixed: `check-be-fe-url-contract.sh` + `fe-build-local-verify.md` + `rules-index.csv` + `output-review-mandate.md` → path đúng; #2 re-run exit 0 clean → CI flipped WARN→HARD STOP. Same path-bug class ở `vn-localization-audit-checklist.md` v1.1.1 (fixed same PR). Persona-sim agent còn tìm bug logo-upload contract drift (request-shape mismatch #2 không cover) → GAP-804.
