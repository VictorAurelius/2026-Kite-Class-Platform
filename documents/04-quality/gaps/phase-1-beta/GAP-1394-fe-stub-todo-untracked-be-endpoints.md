...
| 3 | KH `dashboard/page.tsx:79`(+:42) subscription/health | **PENDING-BE** | `/api/subscription/health` (telemetry tổng hợp tier + usage% + 7-day series) chưa có backend telemetry. tier/trialDaysLeft/instances đã là dữ liệu thật (từ instances API); chỉ sparkline series là stub. Giữ TODO truy vết. |
