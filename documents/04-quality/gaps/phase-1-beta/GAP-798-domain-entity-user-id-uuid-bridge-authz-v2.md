...
- **2026-06-14 — Attendance + Assignment/Grade X-Teacher-Id instances CLOSED → GAP-1300/GAP-1301 (umbrella still PARTIAL):** The GAP-1299 cross-flow sweep (`grep '@RequestHeader("X-Teacher-Id")'`) surfaced the same spoofable-actor-identity class in sister controllers; both now fixed (DONE): **GAP-1300** (attendance — `AttendancePeriodController` + `AttendanceController` + `AttendanceClassBatchController`, 5 sites) and **GAP-1301** (assignment + grade — `AssignmentController` 7 sites + `GradeController.deleteComponent` 1 site). All derive the acting teacher from gateway `X-User-Reference-Id` (`UserContext`) + role-gate writes; spoofable `X-Teacher-Id` header dropped; ADMIN/OWNER bypass via `AuthorizationBean.isAdmin()` (made public GAP-1299, now OWNER-inclusive at grade service layer). Full `kiteclass-core` suite green (1737/0/0, strict-warnings). **GAP-798 stays PARTIAL** — these closed the X-Teacher-Id *spoof* class; the remaining producer-side bridge scope (parent `hasAccessToChild` / `users.reference_id` population / Storage + Assignment-submit + LessonProgress numeric-`X-User-Id` controllers) is unchanged (GAP-798b).

- **2026-06-14 — LMS-authoring instance CLOSED → GAP-1299 (broader bridge still PARTIAL):** The LMS slice of this umbrella (the G1 PoC below) is fixed by **GAP-1299** (DONE): `LmsController` authoring endpoints now (1) role-gated `@PreAuthorize("hasAnyRole('TEACHER','OWNER','ADMIN')")` and (2) derive the acting teacher from gateway `X-User-Reference-Id` (`UserContext`), with the spoofable client `X-Teacher-Id` header dropped entirely; OWNER/ADMIN bypass per-course ownership. `LmsAuthoringAuthzTest` 6/6 + 46 LMS tests green. This is the "Lms" controller named in the AC line "4 controllers (Storage/Assignment-submit/LessonProgress/Lms)". **GAP-798 stays PARTIAL** — the remaining bridge scope (parent `hasAccessToChild` producer side / `users.reference_id` population / Storage + Assignment-submit + LessonProgress controllers) is unchanged (GAP-798b). Note: LessonProgress + Lms GET-identity already migrated to `X-User-Reference-Id` by GAP-1297; GAP-1299 closes the Lms *authoring* authz.

- **2026-06-14 — G1 walk empirical PoC (LMS authoring authz hole confirmed):** G1 runtime walk RBAC+LMS (`documents/04-quality/audits/rst-html/2026-06-14-g1-runtime-walk-rbac-lms.md`) confirms the LMS-controller actor-authz gap described here is **exploitable**. PoC: mint STUDENT token (role=STUDENT) + client-supplied `X-Teacher-Id: 3` → `POST /api/v1/lms/courses/13/modules` → **HTTP 201** (student impersonates teacher 3, creates module). Gateway does NOT strip/verify `X-Teacher-Id` (only X-User-Id/Roles/Reference-Id/Tenant-Id gateway-controlled per GAP-814); `LmsController` authoring endpoints have **0 @PreAuthorize** + trust numeric `X-Teacher-Id` (spoofable — exactly the actor-UUID↔numeric-owner gap). Service check "teacherId must be course owner" passes because attacker sets it = real owner. Same class as GAP-1000 (finalize teacherId self-asserted). AC line "4 controllers (…/LessonProgress/Lms) resolve actor via bridge" closes this. Found during GAP-1113 LMS Increment A teacher-authoring flow walk. NOT filed as separate gap (duplicate of this) per `audit-to-gap-pipeline.md` §2.
