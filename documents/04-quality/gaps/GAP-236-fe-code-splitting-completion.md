...
**Status:** 🟢 DONE 2026-04-28 — Wave GAP-236 closed all 3 Sub-PRs. Sub-PR A (CI bundle budget guardrail) shipped 2026-04-26; Sub-PR B (33 pages converted across 4 parallel agents — A/B/C/D) shipped 2026-04-28 PRs #600/601/602/603; Sub-PR C (analyzer baseline HTMLs for KC + KH) shipped 2026-04-28 in PR #603. Total ≥30 pages AC met (33 actual). Bundle budget green on both apps post-wave.
...
**Related:** Parent GAP-127 (PARTIAL); audit `documents/04-quality/audits/performance/performance-audit-2026-04-19.md`; guide `documents/05-guides/monitoring/frontend-bundle-budget.md`
...
- [x] ≥30 of remaining 44+ pages converted using `next/dynamic` for heavy children components (✅ shipped 2026-04-28 — Sub-PR B Wave: Agent A 7 + Agent B 5 + Agent C 11 + Agent D 10 = **33 pages**)
- [x] Bundle analyzer baseline HTML reports committed (kc + kh) (✅ shipped 2026-04-28 — Wave GAP-236 Agent D Sub-PR C; `documents/04-quality/audits/performance/bundle-analyzer-baseline-{kc,kh}.html`, 749/876 KB raw, both under 5 MB so no compression)
...
- **2026-04-28** — **Wave GAP-236 CLOSED** (consolidation). All 4 agent PRs merged: #600 (Agent B, 5 pages KC admin/attendance/billing), #601 (Agent A, 7 pages KC auth/public — top 3 auth routes −119 KB First Load JS), #602 (Agent C, 11 pages KC classes/courses/students/teachers), #603 (Agent D, 10 pages KH all groups + Sub-PR C analyzer baselines). Total: **33 pages** ≥30 AC threshold. Per-app post-wave: KC max 217 KB on `/classes/[id]/attendance`; KH max 200 KB on `/admin/instances`; both within 250 KB CI budget across all 90 routes (52 KC + 38 KH). Status: 🟡 PARTIAL → 🟢 DONE. Out-of-scope follow-ups surfaced by agents (not filed yet, awaiting triage): (1) `kiteclass-frontend/src/components/ui/calendar.tsx` is unused and a candidate for deletion; (2) HCaptcha lazy-loading on KH `/register` blocked by Next 15 / React 19 ref-forwarding gap (~80 KB potential win); (3) `(auth)/layout.tsx` at 131 KB hoists common chunk into every auth route — provider-hoist refactor could yield more wins.
...
- **2026-04-28** — Sub-PR B Agent C shipped (Wave GAP-236 KiteClass classes/courses/students/teachers bucket). 11 pages converted using `next/dynamic({ ssr: false })`: 8 form pages (classes/[id]/edit, courses/{new,[id]/edit,[id]/classes/new}, students/{new,[id]/edit}, teachers/{new,[id]/edit}) wrapped via 4 new `dynamic-{class,course,student,teacher}-form.tsx` wrappers in `components/forms/`; 2 attendance pages (students/[id]/attendance, classes/[id]/attendance) wrapped via new `components/{class,student}/dynamic-*.tsx` wrappers for `EnhancedAttendanceCalendar` (react-day-picker), `AttendanceDetailDialog`, and `AttendanceFormList`. `pnpm build` green; bundle budget green (52 routes, max 217.64 KB on `/classes/[id]/attendance` — well under 250 KB ceiling); `pnpm test` green (565 passed, 206 pre-skipped). Three integration tests (`{students,teachers,courses}-new`) updated to mock the dynamic wrapper to its real form via `vi.importActual` so synchronous `getByLabelText` queries continue to work in jsdom (deferred async resolution would otherwise force every consumer to migrate to `findBy*`). Per-route deltas: `/students/new` 191 KB (vs prior baseline 234.21 KB top-5 entry — net ~−45 KB on this route family); same pattern across 4 form-page route families.
- **2026-04-28** — Wave GAP-236 Agent D shipped (KH bucket + Sub-PR C). 10 KH pages converted to `next/dynamic` for heavy children: `/(admin)/admin/instances/[id]` (extracted Suspend/Activate/ExtendTrial dialogs into `InstanceActionDialogs.tsx`), `/(customer)/dashboard` (lazy `OnboardingWizard`, only renders when `shouldShow`), `/(customer)/branding/wizard` (4 step components: Upload/Analyze/Generate/Review), `/(customer)/branding/templates` (extracted `TemplatesGrid.tsx` with TemplateCard children), `/(customer)/branding/assets` (extracted `AssetsGrid.tsx`), `/(customer)/branding` (extracted `RecentAssetsGrid.tsx`), `/(customer)/billing/upgrade` (lazy TierSelector + ChangeConfirmation), `/(customer)/billing/payment/[id]` (lazy QRCodeDisplay — only mounts on PENDING), `/(customer)/billing` (lazy PlanComparison), `/(customer)/settings` (lazy 4 tabs: Account/Instance/CustomDomain/DangerZone — 1030 LOC on demand). KH build green; bundle budget green (38 routes, 0 over). Highest KH route = 200 KB (`/admin/instances`). 484 tests pass (4 template-gallery + 2 billing tests adapted to async `findBy*` for lazy components). Sub-PR C: analyzer baseline HTMLs committed — KC `bundle-analyzer-baseline-kc.html` (749 KB) + KH `bundle-analyzer-baseline-kh.html` (876 KB), both under 5 MB, no compression needed.
- **2026-04-26** — Sub-PR A shipped (Wave P2 Cleanup Agent B). CI bundle budget guardrail added to both FE workflows: `scripts/check-bundle-budget.mjs` reads `.next/(app-)build-manifest.json` after `pnpm build`, gzips each chunk per route, fails CI on any route exceeding 250 KB First Load JS. Override via env `BUNDLE_BUDGET_KB` or per-route `bundle-budget.json`. 13 unit tests via `node --test`. Baseline captured: KiteClass top 5 routes = 236.09 / 235.80 / 235.51 / 234.80 / 234.21 KB; KiteHub top 5 = 194.15 / 173.66 / 170.37 / 169.65 / 169.00 KB. All routes within budget. Guide: `documents/05-guides/monitoring/frontend-bundle-budget.md`. Status flipped 🔵 OPEN → 🟡 PARTIAL — Sub-PRs B (44+ page conversions) + C (analyzer baseline HTML reports) still open.
