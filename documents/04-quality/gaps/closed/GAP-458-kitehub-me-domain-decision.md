...
- **Tier 3 cutover automation (PR #1085 OPEN 2026-05-10):** workflow_dispatch + narrow OIDC IAM role + Cloudflare CLI extension. Pre-build Path Y (1-click cutover) song song với fix GAP-459.
- **GAP-459 (filed 2026-05-10):** Activate denial root cause + fix plan (canonical `.vn`→`.me` + LandingShellSSR server component). Domain `.me` choice trong gap này tạo canonical URL drift surface → trigger GAP-459. **Cross-cutting impact:** GAP-412 PARTIAL→DENIED, resubmit gated trên GAP-459 ship.
