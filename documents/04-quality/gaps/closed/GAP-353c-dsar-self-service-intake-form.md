...
**Status:** 🟢 DONE 2026-05-09 (Wave 48 Bucket A cascade — all 11/11 AC verified; GAP-353c-followup-dpo-email-notification shipped DPO + requester email integration via `EmailServiceClient` + 2 Thymeleaf templates).
...
- [x] Email notification flow (DPO + requester acknowledgement) — Wave 48 Bucket A shipped via `EmailServiceClient.sendDsarNewTicketDpoEmail` + `sendDsarAcknowledgementEmail` (outbox-first per `design-patterns.md` §3.5.1 Exception A) + 2 Thymeleaf templates (`dsar-new-ticket-dpo.html`, `dsar-acknowledgement-requester.html`). Try/catch envelope ensures ticket transaction never rolls back on email failure. See GAP-353c-followup-dpo-email-notification (🟢 DONE).
...
- **2026-05-09 (Wave 48 cascade closure):** GAP-353c-followup-dpo-email-notification shipped → last unchecked AC item (line 74 — Email notification flow) satisfied → status flips 🟡 PARTIAL → 🟢 DONE per `gap-done-discipline.md` §2 (all 11 ACs verified). Email integration: 2 new methods on `EmailServiceClient` (re-using outbox-first `dispatchEmail` pipeline) + 2 Thymeleaf templates in `kitehub-email/src/main/resources/templates/emails/` + `DsarServiceImpl` wires both dispatches with try/catch envelopes (email failure does NOT roll back ticket). Verification: `mvn -pl kitehub-subscription verify -P strict-warnings` BUILD SUCCESS (455 tests, 0 failures, 7 DsarServiceImplTest). `BR-PDPL-DSAR-006` (DPO email config) added to `documents/01-business/kitehub/marketing/rules.md` with full 5-attribute review.
