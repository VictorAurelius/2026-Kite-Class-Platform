...
- **GAP-453 (Phase B E2E)** — 🟢 DONE 2026-05-09 (both slices). KH B.2 narrow-subset gate active (PR #1078, `pnpm test:e2e:gates:ci` running route-mocked beta-funnel/ 5 tests, ~9s local / 1m22s CI). KC C.2 direct-navigation gate active (this PR, `pnpm test:e2e:gates` running class-lifecycle.spec.ts 6 tests, ~17s local). Both KC + KH `if: false → if: true`, KC removed `|| true` swallow + continue-on-error so gate is blocking. GAP-403/404/420 unblocked on Phase B path. **GAP-455 (P2)** open for KH coverage extension (user-flagged audit during PR #1078: ~28% scenario coverage, follow-up to ~80% via 7 error-branch tests + mock shape audit, ~2-3h).
