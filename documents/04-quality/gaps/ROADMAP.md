...
3. **GAP-272o** P1 — wizard orchestrator wires `useDeployStream` + `useRegenerateQuota` vào DeployingStep + RegenerateCounter. Filed Wave 34 closure 2026-05-07.
4. **Phase 1 BETA P1 cluster** — GAP-371 CDN Cloudflare + GAP-373 status page + GAP-374 tag-CI automation + GAP-380 staging environment.
...
| **Phase 1 BETA P1 STRONGLY** | 🟡 **PARTIAL** (Wave 33 + 37 SHIPPED) | GAP-371 CDN + GAP-373 status page + GAP-374 tag-CI + GAP-377 smoke test (✅ Wave 26 DONE) + GAP-378 rollback (✅ Wave 25 DONE) + GAP-379 secrets template (🟡 PARTIAL Wave 33) + GAP-380 staging + GAP-403..406 E2E + OWASP (Wave 37) — Wave 37 ships E2E pre-release workflow + 3 beta-funnel specs + visual regression scaffold + OWASP baseline doc (4 of 7 P1 still OPEN: 371/373/374/380) |
| **Wave 37 Release-Hardening 5-layer** | 🟢 **SHIPPED** (2026-05-07) | 22 GAP-NEW: Bucket A Terraform 3/3 DONE + Bucket B Docker 5/5 PARTIAL (pending AWS apply) + Bucket C E2E 2 DONE + 2 PARTIAL (staging-dep) + Bucket D Local dev 3 DONE + 1 PARTIAL (smoke 27GB blocked by GAP-244) + Bucket E AWS cost + Strategy 2 DONE + 4 PARTIAL (human-action). 8 DONE / 14 PARTIAL. AUDIT_DEFER_DOMAIN_MILESTONE per §2.4. Coordinator-applied 2 fixes (pom stubs + pnpm pin) + Bucket D direct ship after Sonnet thrash |
