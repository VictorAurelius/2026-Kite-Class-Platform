...
| 16 | **Wave 20 — GAP-348 Round 3 UI Kits Persona-Driven Review** | GAP-348 (P1) | P1 (pre-port quality gate) | 🟡 IN_PROGRESS 2026-05-05 — Plan PR #802 merged. Bucket A `kiteclass-student` ✅ DONE (avg **100.4/128**, delta -15.6 vs self-report 116, calibration band ✓; verdict APPROVE WITH POLISH; 1 P0 finding payments.html persona violation AC-FIN-001). Bucket B `kitehub-admin` 🔄 background. Bucket C closure pending (synthesis + GAP-348 flip + 1-2 polish gaps if avg <105 per kit + Tier-1 `S-student.md` AC doc gap). |
| 17 | **Wave 21 — Marketing Storytelling kit** | GAP-350 (P2 — promoted from P2 LOWER 2026-05-05 per user request) | P2 | 🟡 ACTIVE 2026-05-05 — Single-bucket: build `ui_kits/kitehub-story-v2/` from Round 1 baseline (archived JSX 546 LOC → HTML kit per Round 2/3 standard). Direction A marketing storytelling for P2 Center Owner trial-signup conversion. Unblocks GAP-275 (Track 2 KH public marketing port) source-of-truth. Closure batched after Wave 20 Part C (avoid `ui_kits/README.md` + `index.html` overlap). Future pair candidate: GAP-274 Phase 1 (KC public marketing kit) — declined for now to keep scope tight. |
