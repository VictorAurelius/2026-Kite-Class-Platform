...
2. **Wave 37 Release-Hardening 5-layer** (recommended next) — 22 GAP-NEW chia 5 layer: Terraform (T1-T3) + Docker release (D1-D5) + Deploy+verify (D1-V4) + Local dev resource (L1-L4) + AWS cost+Free Tier (C1-C4). Architecture B confirmed (~$72/mo Yr1 split EC2). Apply AWS Activate Founders Pack $1k credit → 13.9 tháng cover. Ollama deferred Phase 2 (template-only Phase 1). Audit follow-ups (4 NEW P1: K8s runAsNonRoot + AnalyticsService cold scan + legacy quality-gate dup + admin window.prompt UX) bundle vào Wave 37.
