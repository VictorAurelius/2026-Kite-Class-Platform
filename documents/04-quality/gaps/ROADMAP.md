...
## 🎯 Current Status Snapshot (2026-05-25 — **Wave meta-2 SHIPPED PR #1819 — TestFixtureCleanup dynamic truncate 67% unblock**) — Investigation finding: test profile DISABLES Flyway, dùng Hibernate `ddl-auto: create-drop` (pure-SQL tables không có `@Entity` không tồn tại trong test DB → giải thích PR #1818 fail); Spring KHÔNG autowire `TestExecutionListener` instances (Bucket B's listener cũng silently no-op cùng lý do). Fix shipped: `pg_tables` introspection + listener order 3500 (before TransactionalTestExecutionListener) + `TestContext.getApplicationContext()` bean lookup. **Progress: 6 → 2 fails** (4 CourseSecurityTest SQL injection ✅; EnrollmentIT.shouldIsolate_multiTenantData + InvoiceFlowIT.testMultiTenantIsolation_InvoiceFilters vẫn fail nhưng DIFFERENT mode 404→500 + missing JSON — multi-tenant test logic). **GAP-735 PARTIAL 90%** + **GAP-745 PARTIAL 85%** + **GAP-746 OPEN P2** filed (Wave meta-3 candidate cho 2 stubborn multi-tenant tests).
