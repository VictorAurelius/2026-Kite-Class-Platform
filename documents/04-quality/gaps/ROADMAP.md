...
## 🎯 Current Status Snapshot (2026-06-04 PM — **Wave flow-kh3 G1 PASS coordinator-walked + 10 PRs shipped/in-flight (5 pre-walk merged main + 5 follow-up CI) + 4 new gaps GAP-940/941/942/943 + meta rule starter-kit-upstream-destination v1.0.1**) — Wave flow-kh3 merged main `1ce04fc0`: PR #2151 UC-SUB-01 SUB-20 manual VietQR payment gate (subscription PENDING + Payment.pendingPaymentId carried; tier flip deferred admin-confirm) + PR #2152 GAP-938 admin @PreAuthorize PLATFORM_ADMIN replaces dead X-Admin-Key + PR #2153 GAP-939 Payment account snapshot from VietQRService getters + PR #2155 GAP-937 Mockito UnnecessaryStubbing cleanup + PR #2156 meta rule. **G1 walk evidence:** Owner FREE/TRIAL `g2test-an-8` → POST /api/platform/subscriptions BASIC → HTTP 201 PENDING + pendingPaymentId → admin confirm X-User-Roles=PLATFORM_ADMIN → BASIC ACTIVE + email "Subscription đã kích hoạt" ✅ (post 2 manual workarounds: V62 ALTER + Payment UPDATE — fix PRs shipped). 5 follow-up PRs in flight CI: **#2157** (GAP-942 V62 PENDING schema NOT NULL + CHECK) + **#2158** (GAP-943 application.yml VietQR default match Java @Value) + **#2159** (GAP-940 admin MockMvc 29 IT tests 401/403/200 × 3 controllers) + **#2160** (cleanup unused `instance` var post PR #2151) + **#2161** (GAP-941 kitehub-admin AmbiguousMappingException — root cause Spring AOP collision NOT stale mocks). G2 recipe `2026-06-04-g2-recipe-kh3-subscription.md` updated post-walk với new auth pattern + G3 production parity stub (blocker GAP-612 AWS + GAP-820 paid VietQR). Stack still UP local Docker. Campaign §4 rows: KH-1 ✅ THÔNG (G1+G2+G3) / KH-2c ✅ THÔNG / KH-3 ⬜ → 🔄 eligible post 5-PR merge + clean re-walk. Wave branch local commits (3 docs not pushed): GAP-940 + GAP-941 + G2/G3 update. Next loop = merge 5 PRs + re-walk + KH-3 row flip + KC-1 tenant provisioning. PDPL countdown 28 ngày (2026-07-01).

## 🎯 Previous: Wave flow-kh1 G1 PASS PR #2147 + Wave flow-kh2 G1 PASS PR #2146 SHIPPED — Flow Verification Campaign 2 waves G1 PASS, 5 gaps filed today (2026-06-04 AM) — Campaign flow-kh2 (#2146 merged): KH-2a admin auth + KH-2c owner login + onboarding wizard G1 ✅ via gateway (GAP-916 P0 fix shipped — JwtAuthenticationGatewayFilter Order LOWEST_PRECEDENCE-2); topology revision split KH-2 → KH-2a/b/c per GAP-919 user-flagged FE invite-gated discovery; 3 gaps filed (916/917/918/919). Campaign flow-kh1 (#2147 open): full beta funnel S1-S5 + KH-2c chain G1 ✅ via gateway (anonymous request → admin approve → MailHog email "Mã truy cập Beta KiteHub" → exchange-claim-code → validate token → complete signup → owner+tenant provisioned → onboarding wizard); 1 NEW gap filed [GAP-920](documents/04-quality/gaps/phase-1-beta/GAP-920-api-contract-beta-signup-shape-drift.md) P2 (api-contract.md beta-signup body drift docs vs code BetaSignupCommand). Campaign rows: KH-2a ✅ G1 / KH-1 🔄 walk-pass-pending-human / KH-2c 🔄 walk-pass-pending-human. Stack vẫn UP cho user G2 + G3 verify. Next loop = KH-3 Subscription create + trial→paid migration. PDPL countdown 28 ngày (2026-07-01).
