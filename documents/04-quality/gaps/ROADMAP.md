...
- **GAP-453 (Phase B E2E)** — 🟡 PARTIAL 2026-05-09. KH side DONE: B.2 narrow-subset gate active (`pnpm test:e2e:gates:ci` running route-mocked beta-funnel/ 5 tests, ~9s). KC side deferred to **GAP-454** (P2): KC `class-lifecycle.spec.ts` 2/6 pass narrow subset because helper `navigateToClassDetail()` hits unmocked listing endpoint. **KH coverage extension to GAP-455** (P2): user-flagged audit surfaced gate ships ~28% scenario coverage (5/18 documented business cases); follow-up adds error branches + mock shape audit to reach ~80%. Blocks GAP-403/404/420 closure on KC slice only; KH slice unblocked but shallow.
