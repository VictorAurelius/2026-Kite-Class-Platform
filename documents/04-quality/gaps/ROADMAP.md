...
## 🎯 Current Status Snapshot (2026-06-22 — **Phase-1 closeout `/loop` — 8 gaps closed + CI-hang root fix, 11 PRs merged**) — Multi-round autonomous `/loop` (agent-concurrency-budget-inline-hybrid). **Merged #2517-2527 (11 PR, 0 open):** GAP-154 BRD 27/27 (#2517) · **Quality 88→90 + Security 85→91 cả 2 FAIL→PASS** via GAP-1308 closure (#2518) · api/business audit refresh (#2519) · GAP-1516 (#2521) · GAP-1251 branding docs (#2522) · GAP-664 **3-layer 75/75 domains** (#2523) · GAP-666 README 25→75 (#2524) · **GAP-1393 fix CI treo kiteclass-core tận gốc** — Testcontainers max_connections 100→500 + surefire/CI timeout trip-wire, không treo >30min nữa (#2526) · **GAP-1491 A01 privilege-esc fix LANDED** — @PreAuthorize 8 controllers wide-open (anyRequest().permitAll()) + 26 authz IT, **--admin** (user choice) + trailer ADMIN_MERGE_OVERRIDE:GAP-1393 (CI flaky-slow timeout) (#2525) · GAP-1523 follow-up filed (#2527). **🎯 Phase-1 GATE đạt: Quality 82/100 PASS + 0 P0 local-closable OPEN.** **NEXT (`/clear` rồi `/loop`):** GAP-1523 ƯU TIÊN (kiteclass-core suite ~25min → mọi KC PR risk timeout; nghi *AuthzTest @SpringBootTest) + P2 meta GAP-1492/1509/1522/245. NOT loop-closable: human G2 walk 1066/1115/1139/1213 + ~150 feature gap · counsel 156-D/049/154 · vendor 063/286 · designer 011. Handoff: `documents/03-planning/session-handoffs/2026-06-22-phase1-closeout-loop-8gaps.md`. **Prior (2026-06-21 Business/BRD):** ↓

## 🎯 Current Status Snapshot (2026-06-21 — **Business/BRD cluster + Mobile-OTP — 3 PRs, 6 gaps advanced**) — Phiên `agent-concurrency-budget-inline-hybrid` (2 agents + coordinator inline, 0 idle). **3 PR PENDING MERGE (merge #2513 first):** **#2513** GAP-156 compliance audit 0→70% (baseline audit 75 rules.md + `00-brd/compliance-checklist.md` + **real** 5-attr detector `check-business-rule-attributes.sh` — fixed phantom `audit-gate.py` claim, `business-logic-review.md`→v1.1.0; + SKILL/collect-state inline-hybrid reminder) · **#2514** GAP-063 Zalo channel 20→45% (`ZaloNotificationChannel` bridge + `sms-provider-evaluation.md`) + GAP-154 BRD 66→80% (7 P1 skeleton docs) · **#2515** GAP-286 mobile OTP signup **full-stack** 0→60% (backend `OtpService`+2 endpoints+12 tests + FE `kitehub-frontend /signup/mobile` `pnpm build` EXIT 0 + 3-layer docs). **Cluster Claude-closable P0 đã cạn** — còn lại BLOCKED: human G2 walk (1066/1115/1139/1213+286 E2E) · counsel (156-D/049/154) · vendor (063/286 live ZNS) · designer+budget (011). **NEXT = `/loop` close Phase 1:** merge 3 PRs → triage P0 Claude-closable vs blocked → loop P1/P2 + audit→80, KHÔNG churn blocked; surface human/vendor/counsel items cho user. Handoff: `documents/03-planning/session-handoffs/2026-06-21-business-brd-otp-3pr-loop-prep.md`. **Prior (2026-06-16 agent-G1):** ↓

## 🎯 Current Status Snapshot (2026-06-16 — **Agent-G1 walk KC-1→KC-8 sweep + walk-data committed-seed meta + GAP-1466**) — Phiên agent-G1 (headless Playwright, production-accurate nip.io). **5 PR merged, 0 open:** #2456/#2455 (CI flake re-run + ruff force-exclude) · #2457 (meta `walk-data-committed-seed.md` v1.0.0 + committed idempotent `seed-walk-tenant.sh`) · #2458 (GAP-1466 file) · #2459 (GAP-1466 seed-side fix + seed extend teacher-creates-class + teacher/parent credential). **Agent-G1 sweep:** 8/9 render-clean KC-1/2/3/4/5/7/8 + KC-10/11/12 + RBAC (**0 GAP-1067/1068/1069 recurrence**); KC-8 parent provisioned end-to-end (invite→redeem direct-core→login); KC-6 grade seed-side FIXED+verified (teacher-creates-class → `class.teacher_id`=teacher UUID → enrollments 200, was 403). **Walk baseline reproducible:** `bash kitehub/scripts/seed-walk-tenant.sh` (g2walk owner/teacher/parent creds, idempotent, survive WSL restart). **GAP-1466 PARTIAL:** seed-side fixed; product residual OPEN (owner-created class no teacher-reassign endpoint → escalate P1 if owner-creates model — needs product decision). **NEXT:** human G2★ functional walk KC-1→KC-8 → flip `🔄 → 🟢 THÔNG (local)`; cleanup 4 stale agent-worktree husks (2 có content đáng giá — triage). Handoff: `documents/03-planning/session-handoffs/2026-06-16-agent-g1-walk-kc-seed-meta.md`. **Prior (2026-06-16 Phase-3):** ↓

## 🎯 Current Status Snapshot (2026-06-16 — **Wave flow-fix-1-phase3 COMPLETE — 14 product bug fix + G2 walk PASS**) — Flow Verification Campaign Phase-3: fix **14 product bug** (GAP-1435..1448) từ Phase-2 walk, span 7 flow (KH-5/6/8/9/10 + KC-10/12). 5 bucket disjoint (4 agent Opus + coordinator inline-hybrid; rate-limit lesson 4-concurrent→2/đợt). **PR #2451 MERGED** (commit 120ed4844). Human **G2 walk PASS** (KH `:3001` + KC `sky-education.127.0.0.1.nip.io:3000`) → 14 gaps DONE + closed/. Phase-2 gaps filed PR #2450 (GAP-1435..1454). Meta hardening: `g1-browser-walk` v1.3.0 §3.3 KC local-env access recipe (slug-source + nip.io/.kiteclass.local + GAP-1067 restart) + detector `check-walk-recipe-access-mode.sh` (recurrence #2 access-mode slip). CI fix-wave: 4 tsc test errors (agent caller-sweep miss) + audits-index row + 1 stale DSAR test. **Residual:** GAP-1455 (prod INTERNAL_API_URL AWS-blocked) + GAP-1456 (~34 recipe access-mode WARN-grandfathered) + GAP-1447 SSO future. **NEXT:** 10 flow `🔄 walk-pass-pending-human` (KC-1..8, KC-11, KH-7) chờ human G2★. Handoff: `documents/03-planning/session-handoffs/2026-06-16-phase3-flow-fix-close.md`. **Prior (2026-06-11):** ↓

## 🎯 Current Status Snapshot (2026-06-11 — **Wave ui-kits-100 COMPLETE — tổng hợp toàn bộ gap UI kits, quality-target**) — **Follow-ups 2026-06-12 (user-flagged, merged #2343-#2345):** favicon 237 trang kit + reuse PNG icon-192 (GAP-1229 85% — residual human walk) · banner/chân dung THẬT từ 08-thesis/portrait vào landing-personal+about, carousel đa-banner per GV + hero copy switch + sync persona Khánh GDCD (GAP-1232 DONE v2). Handoff: `documents/03-planning/session-handoffs/2026-06-12-ui-kits-100-complete-handoff.md`. 12 PR merged (#2327 plan + E0 #2329 root-token Inter→Be Vietnam Pro cascade 12 kit + E #2330 sweep tier 23 file/surface/JSON-LD + F #2331 META kit-as-source-of-truth §3.1 + skill kit-production-parity + A #2333 student 100.4→**105.2/128** + B #2334 admin 101.1→**106.2/128** + I-prep #2335 gỡ ThemeSwitcher KH + C #2336 pricing **113** + external verify public **108.8** + H #2337 tier-code PRO→BASIC (V72, fix BASIC-quota bug) + D #2338 wizard v3 ADR-037 + G #2339 favicon chain + I #2340 theme demo 68 file 4 KC kits). **14/15 gap DONE** (363b/363/364b/364/348/428/274/1212/1098/366/367/1228/1230 + umbrella) + GAP-1229 PARTIAL 85 (residual: human browser walk upload favicon — gate §2.5(b) exception). **Track 2 ports GAP-269/271 UNBLOCKED.** Closure: `wave-2026-06-11-ui-kits-100.md` §7.1 reconciliation + quality-target gate verdict. **Prior (2026-06-09):** (2026-06-09 — **Wave landing-100 COMPLETE — KC landing page 7 buckets**) — autonomous wave per Stop-hook goal. **7 buckets** integrated (cherry-pick + conflict resolution, built green): A empty-state sections ✅ + B brand/contact (🟡 GAP-1083 BE fields) + C per-tenant SEO/OG ✅ + D Đợt1 slot wiring ✅ + E HeroSection dedup ✅ + F Problem/HowItWorks/TrustStrip/FloatingCTA + Zalo CTA (🟡 GAP-1083) + G ensure-bucket + demo-trio seed ✅. **G1-headless verified:** curl render 200 + `?tenant=` data-binding proven + seeder verified (DB user=kitehub, DB=kiteclass_shared). Per-tenant subdomain rendering gated GAP-811/1077 (out-of-scope per plan §7.1). **KH-3 G2 walk blockers fixed same session:** subscription /active 400→404 (GAP-1079, EntityNotFoundException) + FE bare-shape 5 hooks (use-subscriptions) + payment page (use-payments `data.data`→`data`, cross-flow sweep) + seeder template_type NOT NULL + BrandingDataSeederTest 8-arg ctor. **New gaps filed:** GAP-1079/1080/1082/1083. **New meta:** rule `kitehub-kiteclass-boundary.md` v1.0.0 (KH `:3001` vs KC `:3000` canonical matrix) + memory. Commits chain 34806286→12fb8a01 + 8e4ca3a5. Handoff: `documents/03-planning/session-handoffs/2026-06-09-landing-100-complete.md`. **Prior (2026-06-07 late — 3 waves: g2-blockers-1 + p0-prov-1 + p0-ux-1 — P0 28→20):** P0-over-G2 drive-down continued. **(1) g2-blockers-1** (#2244-2248): merged 5 PR; fixed CI on #2245 (`SubscriptionRenewalServiceTest` NPE — GAP-1016 renewal payment-gate) + #2246 (9 stale-fixture tests — GAP-1004 over-payment guard + GAP-1000 grade authz-from-principal); test-only, production not weakened. **(2) p0-prov-1** (#2249, walk-convergent): **7 gap DONE** (GAP-942/945/946/948/949/1053/1054) via 2 prep agents + coordinator gateway walks — GAP-942 POST /subscriptions 201/PENDING, GAP-948 MailHog tenant-ready render, GAP-949 3 audit rows, GAP-945 markProvisioned PENDING→TRIAL, GAP-946 0 half-provisioned; **GAP-1055 filed** (provisionInfrastructure real-impl Phase 1.5); 0 walk-surfaced bug. **(3) p0-ux-1** (#2250, local-verifiable UX): **GAP-297/950/951 DONE** — batch invoice (V93 + idempotency + pro-rata; live prorated 1.3M), onboarding sample-data (1GV+1lớp+3HS live), mobile-first admin (Playwright 5/5). State-check caught GAP-950 wizard + GAP-951 sidebar pre-existing (scope-revise). Vendor-gated GAP-063/286 (Zalo/SMS) + GAP-975/976 (SePay) defer — real-user action (đăng ký vendor account). **Phase 1 BETA P0 28→20.** **NEXT:** GAP-885 RLS coverage (local-buildable) OR đợi user setup vendor (SePay/Zalo) unblock 975/976 + 063/286 OR AWS-gated cluster (`start-stack.sh`). Handoff: `documents/03-planning/session-handoffs/2026-06-07-p0-drive-down-3-waves.md`. **Prior (wave-p0-2):** ↓

- **2026-06-12 — Wave landing-100 COMPLETE (G2★ PASS):** Human walk nip.io production-accurate PASS — lần đầu gate G2★-absorbs-G3-functional chạy trọn. Session 06-12: 11 PR (#2347-2354 + 3 kit PR): GAP-1233 kit click-through + GAP-1234 wizard bước Thông tin + 3 màn editor GAP-815 design + GAP-1236 v3 reachable + GAP-1221 contact email/subject optional (V97, 201/400 walk-verified) + GAP-1222 defer trailer + GAP-1235 chân dung thật/carousel 4-4-3/aboutText (seed-pack) + GAP-1077 DONE + branding-100 plan reconcile (#2349: +GAP-1231/1211, 272c/272e reuse-note, trục content). Re-score ~90/100 đạt gate. Reconciliation `wave-2026-06-09-landing-100.md` §7.2. Handoff: `2026-06-12-landing-100-complete-g2-pass.md`. **NEXT: branding-100 execute** (16 gaps; Bucket A kit v3 redesign §2.5 trước; GAP-1213 P0 deploy=MOCK).
- **2026-06-11 landing-100 fix-pack (merged #2326):** 25 gaps xử lý (19+ DONE) + 2 kit mới (landing-personal 113/128, kiteclass-public 4×110-115) + multi-banner V96 + port 4 trang public + META wave-closure v1.2.0 (fix-hết-không-kể-phase). branding-100 plan draft (3 audit; GAP-1213 P0 deploy=MOCK).

## 🎯 Current Status Snapshot (2026-06-07 night — **Wave p0-2 verify cluster — §2.6.1 dogfood**) — PR #2242. **GAP-942 🟡 PARTIAL 0→40%** (V62 + PENDING semantics already shipped PR #2157 per §2.8; AC#1 verified live `started_at/expires_at nullable=YES`; runtime POST /subscriptions walk + regression IT residual). **GAP-975/976 verify-walk deferred** (`SEPAY_API_KEY` empty local — webhook auth walk needs config+restart). **GAP-1054 filed** P2 (agent-drafted `SubscriptionPendingNullableColumnsIT` FK-parent-instance seed bug). **§2.6.1 fired 4th time this session** — GAP-882/946/948/942 all "near-done" on paper but code already shipped; residual = runtime-walk/template/config, NOT new code. P0 stays **28**. **NEXT (fresh context):** GAP-975/976 SEPAY-key walk + GAP-942 subscription POST walk + GAP-1053 template + GAP-885 RLS. Handoff: `documents/03-planning/session-handoffs/2026-06-07-wave-p0-1-p0-2-meta-2.6.1.md`. **Prior (wave-p0-1):** ↓

## 🎯 Status Snapshot (2026-06-07 — **Wave p0-1 local P0 cluster + META §2.6.1**) — P0-over-G2 pivot. Wave p0-1 (3 disjoint buckets, octopus-merge PR #2241): **GAP-882 🟢 DONE** (Bucket A — fix-time state-check found V86 pre-fixed `invoices.status`; genuine drift = `invoice_items.item_type` no-CHECK → V92 `chk_invoice_items_type` + backfill; IT 1635 0-fail + G3 live DB walk accept-UPPERCASE/reject-lowercase). **GAP-946 🟡 PARTIAL 55%** (Bucket B — defensive `assertDatabaseProvisioned()` × 3 InstanceService sites; `provisionInfrastructure` real-impl = separate wave). **GAP-948 🟡 PARTIAL 70%** (Bucket C — code 100% shipped W-prov-1; G3 walk verified wiring live tenant.deployed→consumer→outbox→EmailConsumer→kitehub-email-received; MailHog residual = tenant-ready Thymeleaf template render error → filed **GAP-1053** P1). **Fix-time state-check (`audit-to-gap-pipeline.md` §2.8) revised all 3 buckets** vs plan — work already shipped → **META fix PR #2240**: new §2.6.1 Bucket-Completion Check (v1.5.0, classify 🆕 Greenfield/🔨 Delta/⚠️ Already-shipped at wave-plan-time; symbol-exists ≠ work-remaining) + `_TEMPLATE.md` §4.1. Phase 1 BETA P0 **29→28** (GAP-882). **NEXT:** GAP-1053 template fix → close GAP-948 AC#2; next P0 local-verifiable cluster (GAP-975/976/610 verify-walk + GAP-885 RLS wave-p0-2). **Prior (night-2):** provisioning-1 8 buckets merged ↓

## 🎯 Status Snapshot (2026-06-06 night-2 — **provisioning-1 ALL 8 BUCKETS CODE-MERGED**) — Wave provisioning-1 tenant-provisioning KC saga cluster (8 P0) executed via dependency-ordered DAG + parallel Opus worktree agents. **8/8 buckets merged** (A keystone #2214 saga wiring → F #2213 TenantSettings(V90) + B #2216 audit + G #2215 PDPL cascade(V91) + D #2217 alert/sweep + C #2219 tenant-ready email + E #2218 admin retry). Keystone GAP-945 (`tenant.created` publisher + `TenantCreatedEventConsumer` wiring orphan `TenantProvisioningSaga` + Testcontainers round-trip IT) froze contract → fan-out. Manual merge resolves: V90↔V91 migration collision + `TenantAuditService` 3-method consolidation + gap-status.csv 3-way; rebased G+E local-verified per `admin-merge-discipline.md` §2. **8 gaps GAP-945/946/947/948/949/952/953/954 all 🟡 PARTIAL 40-90%** — single flip-to-DONE gate = **KC-1 live full-stack walk** (Docker subscription+core+rabbit+postgres+minio, pre-walk persona sim per `pre-walk-persona-simulation-mandate.md`). Per-gap follow-ups: GAP-945 status callback / GAP-949 AFTER_COMMIT FK / GAP-952 CloudWatch live-apply (GAP-612) / GAP-953 retry idempotency. Reconciliation table: wave plan §7.1. **NEXT:** KC-1 live walk to flip 8 gaps DONE. **Prior (night):** 7/7 P0 security cluster CLOSED + provisioning-1 started ↓

## 🎯 Current Status Snapshot (2026-06-06 night — **7/7 P0 SECURITY CLUSTER CLOSED + provisioning-1 epic started**) — Security: Bucket C (#2207 GAP-1025/1035 @PreAuthorize authz) + Bucket B (#2209 GAP-1015/1019/1023 cross-tenant IDOR) → **7/7 P0 security cluster from Flow Verification Campaign G1 walks CLOSED**. IDOR fix = service/controller-layer `TenantOwnershipGuard` (admin bypass via SecurityContext, AccessDeniedException→403); fix-time state-check (`audit-to-gap-pipeline.md` §2.8) found gateway `TenantHeaderGuardFilter` ALREADY injects trusted `X-Tenant-Id` → **no gateway change needed**. `/expiring` → admin-only (list-all leak sweep). Caller-sweep migrated 3 test classes; subscription+branding full `mvn test` BUILD SUCCESS 0-fail. **Provisioning epic:** wave plan #2210 (8-gap KC saga cluster, dependency-ordered Foundation A → Phase 2 parallel B-E → Phase 3 F/G; §2.5 rescoped 4 over-claiming gaps GAP-948/949/953/954 → PARTIAL). Bucket A Phase 1a #2211 (GAP-946 fail-fast — removed 3 InstanceService silent-swallow sites → `provisionDatabase` failure propagates, `@Transactional` rollback no `databaseUrl='pending'` corruption; GAP-946 PARTIAL 40%). Filed GAP-1044 (stale `*IT` not CI-run — no failsafe → surefire excludes). **NEXT:** GAP-945 saga keystone (tenant.created publisher + `@RabbitListener` wiring orphan `TenantProvisioningSaga` + Testcontainers RMQ) → then Phase 2/3 wave-pack parallel agents. Handoff: `documents/03-planning/session-handoffs/2026-06-06-night-security-closed-provisioning-1-started.md`.

## 🎯 Previous Snapshot (2026-06-06 — **Wave auth-2 post-audit follow-ups CLOSED** — 3 parallel Opus buckets) — Fixed 5 remaining auth-1 audit gaps via wave-pack (plan PR #2190 → Bucket A docs #2192 / Bucket B kc-core auth #2193 / Bucket C devops #2191 → closure). **GAP-1009 DONE** (tenant-auth 3-layer docs + parent/student-portal Option B sync + BR-PARENT-004) · **GAP-1010 DONE** (5 test classes/49 tests incl AuthCredentialPostgresIT on Testcontainers Flyway schema) · **GAP-1011 DONE** Option A (provision/setPassword 409 AUTH_EMAIL_CROSS_TENANT; doc 1-email-1-tenant Phase 1) · **GAP-1013 DONE** (setPassword cross-entity reject + disableCredential + AuthPasswordPolicy + PII mask + jti + timing) · **GAP-1014 PARTIAL 60%** (secrets.tf HS512 desc + PARENT_PORTAL_ENABLED prod override shipped; kc-core prod compose DEFERRED blocked-by GAP-444 Phase 7 per user decision). Decisions locked via AskUserQuestion: GAP-1011 Option A conservative + GAP-1014 defer-compose. CI fix Bucket B: gitleaks false-positive on canonical test-HS512-hex → `.gitleaks.toml` allowlist regex (root-cause: gitleaks-action scans full PR commit range, inline-allow insufficient). **Note (user 2026-06-06):** wave-auth-2 = debt cleanup, KHÔNG đóng góp Flow Verification Campaign — campaign trọng tâm là G1-all-flows-first → G2 (see `flow-verification-campaign.md` §4 + memory `project_flow_campaign_g1_first_then_g2`).

## 🎯 Previous Snapshot (2026-06-06 — **Wave auth-1 KC-native login MERGED PR #2186** `2b01ac93`) — Parent+Teacher persona auth pulled forward from Phase 2 + shipped end-to-end: `POST /api/v1/tenant-auth/login` (HS512 JWT role+tenantId+referenceId claims) + gateway `X-User-Reference-Id` inject + anti-spoof (Bucket C) + parent provisioning via redeem + teacher credential endpoint (Hướng B). G3 chain verified (real parent, gateway :9000): login→facet 200 + IDOR 403 + anti-spoof; teacher login role=TEACHER. **CI fix (this session):** 4 gates were red on PR #2186 from 2 auth-1 root causes — (1) `JWT_SECRET` missing at context boot → `AuthTokenService` fail-fast (fixed: test `jwt.secret` in `application.yml` + `--jwt.secret` in `check-schema-drift.sh`); (2) `auth_credentials` flagged RLS_DISABLED (fixed: exempt in `check-rls-coverage.sh` — V89 intentionally pre-auth lookup). All gates green → merged. Gap sync: **GAP-798b** OPEN→PARTIAL 17% (gateway X-User-Reference-Id forward shipped; kitehub `users.reference_id` + 4-controller sweep remain); **GAP-725** PLANNED parent+teacher shipped (student login + OTP Hướng C remain Phase 2). **Post-wave audit suite COMPLETE** (per `post-wave-audit-mandate.md`) — business-logic **64/100 C**, api-contract **85/100 B**, ops-readiness **71/100 C** (all PARTIAL, **no P0** — confirms merge was safe). 27 findings → 6 gaps filed [GAP-1009](documents/04-quality/gaps/phase-1-beta/GAP-1009-auth-1-business-doc-completeness.md) (tenant-auth 3-layer docs) / GAP-1010 (auth test coverage) / GAP-1011 (multi-tenant email-unique collision) / GAP-1012 (kc-tenant-auth rate-limit + HS512 keylen — ✅ **DONE** PR #2189, live-429 verified) / GAP-1013 (credential hardening cluster P2) / GAP-1014 (kc-core prod deploy parity). Reports in `documents/04-quality/audits/{business-logic,api-contract,ops-readiness}/2026-06-06-wave-auth-1-*.md`. **Remaining auth:** student provisioning + KC-9 build (Bucket E) + OTP Hướng C (Zalo/SMS vendor-dependent) + production parity (kiteclass-core prod JWT_SECRET via fetch-secrets.sh + IaC). G2 human walks still pending KC-1..8.

## 🎯 KC-1 pre-walk outside-in audit (2026-06-04 PM-late) — **27 canonical gaps filed GAP-945..GAP-971 from 3-agent consensus on 52 raw findings** — Wave flow-kh3 G2 ongoing; KC-1 walk DEFERRED until flow-kh3 finish per user direction. 3-agent audit (persona 20 + matrix 27 + benchmark 5+5) surfaced 52 raw → 27 canonical: P0 ×10 / P1 ×11 / P2 ×5 / P3 ×1. KC-1 walk verdict: ❌ NOT READY — 4 P0 missing (tenant.created publisher / TenantSettings entity / tenant-ready email / onboarding wizard) MUST close pre-walk. Existing gaps EXTEND/REFER: GAP-466/531/532/535/536/201. Audit artifacts: `documents/04-quality/audits/persona-review/2026-06-04-pre-walk-kc1-*.md`. Defer-to: all KC-1 gaps marked "After Wave flow-kh3 finish" per user priority order 2026-06-04.

## 🎯 Current Status Snapshot (2026-06-04 — **Wave flow-kh1 G1 PASS PR #2147 + Wave flow-kh2 G1 PASS PR #2146 SHIPPED — Flow Verification Campaign 2 waves G1 PASS, 5 gaps filed today**) — Campaign flow-kh2 (#2146 merged): KH-2a admin auth + KH-2c owner login + onboarding wizard G1 ✅ via gateway (GAP-916 P0 fix shipped — JwtAuthenticationGatewayFilter Order LOWEST_PRECEDENCE-2); topology revision split KH-2 → KH-2a/b/c per GAP-919 user-flagged FE invite-gated discovery; 3 gaps filed (916/917/918/919). Campaign flow-kh1 (#2147 open): full beta funnel S1-S5 + KH-2c chain G1 ✅ via gateway (anonymous request → admin approve → MailHog email "Mã truy cập Beta KiteHub" → exchange-claim-code → validate token → complete signup → owner+tenant provisioned → onboarding wizard); 1 NEW gap filed [GAP-920](documents/04-quality/gaps/phase-1-beta/GAP-920-api-contract-beta-signup-shape-drift.md) P2 (api-contract.md beta-signup body drift docs vs code BetaSignupCommand). Campaign rows: KH-2a ✅ G1 / KH-1 🔄 walk-pass-pending-human / KH-2c 🔄 walk-pass-pending-human. Stack vẫn UP cho user G2 + G3 verify. Next loop = KH-3 Subscription create + trial→paid migration. PDPL countdown 28 ngày (2026-07-01).

**Wave security-1 (2026-06-05) — P0 GAP-983 cross-tenant by-id leak FIXED + closed.** Enable Hibernate `tenantFilter` trên transaction-bound session (`TenantAwareDataSourceInterceptor` extension; OSIV-off + `@Transactional` mở session riêng mà request-interceptor không reach = root cause). Spike: MappedSuperclass `@Filter` suffices → Bucket B 58-entity sweep KHÔNG cần; Bucket C exception→404 already-correct. G1 IT (ClassTenantFilterTransactionIT 4/4 RED→GREEN + TenantIsolationIT 3/3 closes GAP-362) + G2 live re-walk PASS (khanh-phapluat → 404 / sky → 200 trên classes/sessions/courses/teachers by-id). Closed: GAP-983 + GAP-362. Follow-ups: GAP-985 (RLS defense-in-depth hở P1), GAP-986 (Redis cache @class 500 + leak residue P2), GAP-987 (RLS untestable test profile P2). Reconciled: GAP-746→70% / GAP-749→60% (per-repo sweep superseded by centralized fix). KC-3 campaign §4 unblocked → re-verdict pending G2-handoff.

## 🎯 Previous: Wave local-doable-14 PR #2134 SHIPPED — Bucket B+C+E entity sync + audit-UUID + DB CI gates (2026-06-03) — 17 commits merged (Codex WIP B+C+E + 2 caller-sweep fixes for GAP-891 `tenant_id`→`tenant_slug` rename: `BrandingDataSeeder.java:170` + `Wave02MigrationsTest.java:117`) — 17/37 Wave 13 anomaly gaps (GAP-874-910) closed via this PR: 6 DONE (GAP-874/875/876/880/881/890 entity-drift) + 11 PARTIAL ≥85% (GAP-878/879/882/884/887/891/895/899/900/901/904/908/909/910). 9 V-migrations shipped (V79-V86 + CI gates). Post-merge audit suite ✅ ALL PASS — `api-contract-audit` **94/100 A** (0 regressions, schema-sync only) + `ai-branding-quality-gate` **62/100 NOTES** (carry-forward, scope no §0 triggers). Wave 14 NOT yet closed (5-bucket wave; only B+C+E shipped + parts of A/D bundled; remaining work Bucket A KH RLS sweep + Bucket D KH money harmonize → GAP-912 defer-marked).

## 🎯 Previous: Wave local-doable-8 Bucket C SHIPPED — GAP-622 closed** (2026-06-02) — pre-launch readiness consolidation doc) — `documents/03-planning/roadmap/pre-launch-readiness-consolidation.md` shipped consolidating Phase 1 BETA gate blockers: §1 10-gate criteria (5 PASS / 4 PARTIAL / 1 BLOCKED) + §2 24 active P0 catalog (state-check empirical 2026-06-02 — KHÔNG 27 như GAP-622 filed claim 2026-05-18; GAP-612 closed 2026-05-26 + 2 flipped DONE post-filing) + §3 Mermaid dependency graph (AWS DONE → deploy → tenants critical path) + §4 unblock paths A/B/C (deploy + PDPL + quality polish) + §5 cross-links canonical sources + §6 decision tree + §7 cadence ownership. **PDPL countdown 29 ngày** (2026-07-01); **Quality 90/110 B+ PASS** BETA gate ≥80. Critical path: GAP-866 + GAP-502 + GAP-756 → 5 tenants live → 14 ngày observation = ~3 tuần wall-clock to v1.0.0-rc.

## 🎯 Previous: Campaign local-doable-1 SHIPPED — 6 gap #2037-2042**) — Autonomous /loop local-doable gap fix session 1. **3 DONE** (GAP-727 verification-only Testcontainers IT / GAP-827 landing input sanitize 25 unit+3 IT / GAP-599 JWT 2-tab Playwright 3/3) + **3 PARTIAL honest** (GAP-695 catalog refresh 85% — steering output 26 local-doable / 11 AWS-blocked; GAP-610 beta-signup token H4 fix 95% RST walk; GAP-543 email VN content+MailHog 95%). Sweep filed GAP-824/829/830/543.4. Verify all LOCAL (Testcontainers + MailHog + Playwright); 5% remainders = AWS/SES live-verify blocked GAP-612. Lesson: parallel-agent gap-status.csv conflict + GAP-829 ID collision → rebase+renumber GAP-830. Stack rebuilt local `up.sh --rebuild`. **Next loop cluster:** GAP-536/729/777/726/774/775 (disjoint local-doable per GAP-695 steering).

## 🎯 Previous: Wave beta-readiness-9 SHIPPED — 4/4 parallel buckets**) — Track B Phase 1 BETA gate. 4 parallel Opus agents (PR #2017 plan + #2018/2019/2021/2022 + landing-kit #2020). **Notable: 3/4 buckets stale gaps — agent state-check (`audit-to-gap-pipeline.md` §2.8) prevented re-implement/regression.** A **GAP-814** PARTIAL 75% (header-strip pre-shipped #1991; delivered 27-route tenant-coverage audit + filed **GAP-825** JWT-sig+isolation). B **GAP-772+773** DONE-SUPERSEDED (staff-invite canonical kitehub-subscription Wave 80; kiteclass-core reversed GAP-786; agent correctly NOT rebuilt). C **GAP-127** PARTIAL 85% (real fix +@tanstack/react-table optimizePackageImports; both build pass). D **GAP-658** PARTIAL 90% (already VN Wave 98 B2; residual en-US = intentional BR-SEED-001 fixtures). Also shipped **landing review kit** (#2020 — design brief + /100 checklist for KiteClass landing redesign). Prior: Wave onboarding-polish-2 (GAP-536/538/610) + GAP-727/732 authz. **Next:** AWS-up session — live verify GAP-814/536/610/658 + GAP-825 + GAP-659 copywriter + GAP-823 META instances-triad (ADR); Wave thesis-2 NFR; Wave meta-9 candidate sau 30-day grace.
