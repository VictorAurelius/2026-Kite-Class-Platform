...
| 14 | **UI Kits Track 2 production port** (multi-wave) | GAP-273 (12 components shared lib — BLOCKING) + GAP-266..272 (7 kit ports) | P2 (UX growth) | 🔵 OPEN 2026-04-29 — User accepted Round 3 quality 2026-04-29. **Recommended sequence:** GAP-273 FIRST (sliced into 3 component buckets), then 7 kit ports (each 1-2 week wave). Total estimate ~10-15 weeks across multiple waves. Each kit port = wave-pack candidate when sliced into auth+nav / detail-screens / domain-feature / polish. Decision deferred to wave kickoff: shared package vs per-app duplication (Option A/B/C in GAP-273). **⚠️ Coverage audit pending (Wave UI Coverage Audit — Cluster 15) — additional GAP-274..279 likely after audit identifies uncovered public/auth/error/modal scope.** |
| 15 | **UI Coverage Audit wave** | (audit-only, spawns GAP-274..279 follow-ups) | Meta — evidence preservation per `output-review-mandate.md` §1 | 🟡 ACTIVE 2026-04-29 — Wave plan: `documents/03-planning/waves/wave-2026-04-29-ui-coverage-audit.md`. 2-agent parallel + coordinator synthesis (~85 min target). Audits 100% production frontend (40 KC pages + 24 KH pages + ~16 modals + 19 component subdirs + error pages + layouts) cross-ref against R2+R3 HTML kit coverage. Output: `documents/04-quality/audits/2026-04-29-frontend-ui-coverage-audit.md` + dossier `12-modal-dialog-inventory-{kc,kh}.md` + `14-common-components-inventory-{kc,kh}.md` + `15-error-layout-inventory.md` + GAP-274..279 (or revised list) referencing audit as evidence. |
