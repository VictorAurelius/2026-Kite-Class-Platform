...
**Wave 26 Bucket C in flight 2026-05-06 (GAP-377 smoke test post-deploy automation — extension on GAP-089 baseline).** `scripts/smoke-test.sh` extended 265 → ~383 LOC: dual-URL support `<KH-url> <KC-url>` (0/1/2 args; 1-arg legacy backward-compat), 18 assertions (4 health + 4 pages /legal/{privacy,terms,cookies} + 2 auth substitutes /login + /register + 1 KH `/api/health` + 1 ConsentBanner body-contains + 2 KC public APIs + 2 error-handling + 2 gateway no-502), build-info echo via `/actuator/info`. Refactored body-buffer to file-based to fix latent `set -u` BODY unbound-var bug (subshell scope issue with `code=$(http_get URL)`). Wired into `.github/workflows/deploy-staging.yml` "Post-deploy smoke test (GAP-377)" step consuming `vars.STAGING_KH_URL`/`STAGING_KC_URL` with fallback defaults; cross-link added at `release-1-deploy-plan.md` §2.4 + §3.3. State-check verdict: `/auth/signup`, `/auth/request-beta-access`, `/api/v1/health` absent in kitehub-frontend → substituted `/login`, `/register`, `/api/health`; follow-up `GAP-377-followup-auth-route-checks` (P3) filed for re-alignment if those routes ship. Shellcheck same level as baseline (3 SC2059 info-only preserved; no new errors/warnings). **GAP-377 🔵 OPEN → 🟢 DONE** per `gap-done-discipline.md` §2. Counts: 167 → **167 OPEN** (-GAP-377 closed; +GAP-377-followup-auth-route-checks).
