...
4. **🚨 IMMEDIATE NEXT (post-Wave-38)** — Review + merge **PR #947 release-1-deploy-runbook DRAFT** → ship **Phase 0 plan refresh** (Oracle→AWS Singapore Architecture B in `release-1-deploy-plan.md`, archive `kitehub-oracle-cloud-deployment.md`). Plan refresh BLOCKING user-action Phase 1 (domain registration, AWS account, Cloudflare, SES, Statuspage). Sau Phase 0 → user begins Phase 1 user-actions per runbook.
