...
**Status:** 🟢 DONE — Wave 6 closed 2026-04-26 (Sub-PR 6.5). Audit skill shipped + scored 82/100 Grade B (was 70 baseline). 1 hotspot refactored (TrialToPaidService 546→385). Remaining anti-patterns transferred to GAP-222a/b/c (Outbox migration) + future P3 (Student Primitive Obsession)
...
| Quality audit "Patterns applied correctly?" section | ✅ DONE | `quality/design-pattern-audit/SKILL.md` shipped Sub-PR 6.0b (#543); ran baseline 70/100 (#544) and closure 82/100 Grade B (Sub-PR 6.5) |
| Refactor existing anti-patterns (God Service, primitive obsession) | ✅ DONE (1 hotspot) + ⏭️ DEFERRED (rest tracked) | TrialToPaidService 546 → **385 LOC** via Facade extract (Sub-PR 6.2 #545, 3 collaborators: MigrationStateMachine 80 LOC, MigrationEventEmitter 48 LOC, MigrationRetryRunner 153 LOC, 18 isolated tests). InstanceService 496 LOC SKIPPED (under §3.1 500-LOC threshold; preventive refactor rejected per YAGNI — Sub-PR 6.3 SKIPPED with justification in Wave 6 plan §2.1). Outbox bypass migration tracked GAP-222a/b/c. Student/Branding/ContactMessage Primitive Obsession deferred to P3 cleanup gap (when value-object library lands) |
...
- **2026-04-26 — Sub-PR 6.5 Wave 6 closure** shipped: re-ran design-pattern-audit (score **70 → 82** Grade C → B, +12). Cat 1 +6 (TrialToPaid refactor), Cat 4 +4 (Config-filter calibration eliminating bean-wiring false-positives), Cat 5 +2 (§3.5.1 Exception A marker recognition). ADR-020 ACCEPTED (`/client/` package = adapter convention, no rename). InstanceService 496 LOC SKIPPED (Sub-PR 6.3) per YAGNI + §3.1 threshold rule. Audit suite verified: post-wave Quality refresh (no regression — code change was pure refactor + test additions); Performance audit not triggered per `audit-gate.py` AUDIT_RULES (no DB query path / API handler signature change). **Status: 🟡 PARTIAL → 🟢 DONE.** Remaining open work transferred to GAP-222a/b/c (Outbox migration) — separate roadmap items, not Wave 6 scope.
