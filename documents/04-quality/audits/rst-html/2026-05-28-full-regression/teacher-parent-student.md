...
verdict: BLOCKED — 18/23 NEEDS-DATA (persona accounts + tenant data un-seeded), 2 DEFERRED, 1 NEEDS-USER-BROWSER; 3 real bugs surfaced (tenant-resolution + NoHandler-as-500 + CSV path drift)
bugs_surfaced: 3 (1 P0 tenant-resolution, 1 P1 error-mapping, 1 P2 CSV doc drift)
...
1. **P0** — Tenant context KHÔNG resolve từ JWT cho list endpoints KiteClass-core (cần `X-Tenant-Id` header thủ công).
2. **P1** — `NoHandlerFoundException` (404) bị trả về thành `500 SYSTEM_INTERNAL_ERROR`.
3. **P2** — CSV ghi sai path EMAIL-VERIFY (`/api/v1/auth/verify-email` 404; path thật là `/api/auth/verify-email`).
...
|---|---|---|---|---|
| 1 | **Tenant context KHÔNG resolve từ JWT cho list endpoints KiteClass-core**. `GET /api/v1/teachers` không header → `[]` rỗng; cùng request + `X-Tenant-Id: 877dff9d...` → trả teacher `Trần Quốc Bảo`. Log core hiển thị `tenant=-` (không resolve). Mọi flow Teacher/Parent đọc dữ liệu sẽ thấy rỗng nếu FE không gắn `X-Tenant-Id`. | **P0** | curl 2 lần (có/không header) trả khác nhau; log `[tenant=- trace=...]` | Gateway TenantResolver phải derive tenantId từ JWT `tenantId` claim → forward `X-Tenant-Id` xuống core. Hoặc core đọc tenant từ JWT thay vì chỉ header. Sweep mọi list endpoint. (Trùng bug-class với Wave meta-6 Bucket A Bug #16 + Bug #13 — recurrence). |
| 2 | **`NoHandlerFoundException` (404) trả về thành `500 SYSTEM_INTERNAL_ERROR`**. `/api/v1/classes`, `/api/v1/teacher/dashboard`, `/api/v1/parent`, `/api/v1/attendance`, `/api/v1/grades`, `/api/v1/enrollments`, `/api/v1/assignments`, `/api/v1/invoices` — tất cả base-path GET không handler → log `NoHandlerFoundException` nhưng response body `code=SYSTEM_INTERNAL_ERROR` status 500. | **P1** | Log core: `Resolved [NoHandlerFoundException: No endpoint GET /api/v1/parent]` + body `{"code":"SYSTEM_INTERNAL_ERROR",...}` status 500 | GlobalExceptionHandler phải map `NoHandlerFoundException` → 404 (RFC 7807 `Not Found`), không phải 500. Sai status che giấu "endpoint không tồn tại" thành "lỗi hệ thống" → khó debug + sai semantics cho FE retry logic. |
| 3 | **CSV ghi sai path EMAIL-VERIFY**. `verify_via`/expected dùng `/api/v1/auth/verify-email` + `/api/v1/auth/resend-verification` → cả 2 trả **404**. Path thật: `/api/auth/verify-email` + `/api/auth/resend-verification` (AuthController `@RequestMapping("/api/auth")`, KHÔNG có `/v1`). | **P2** | curl: `/api/v1/auth/verify-email` → 404 "Endpoint not found"; `/api/auth/verify-email` → 400 (token invalid, route OK) | Sửa CSV rows EMAIL-VERIFY-002/004 path từ `/api/v1/auth/...` → `/api/auth/...`. Doc drift, không phải lỗi code. |
