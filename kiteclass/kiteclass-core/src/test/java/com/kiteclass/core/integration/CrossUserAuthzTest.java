...
class CrossUserAuthzTest {
    ...
    @Test
    @Disabled(
        "AUDIT FINDING A01-CLASS-01: classes.teacher_id column absent from test schema " +
        "(Flyway disabled, Class entity does not map teacher_id). " +
        "ClassServiceImpl.createClass() also never sets teacher_id in production — " +
        "guard would deny ALL teachers (lock-out, not IDOR). " +
        "GAP candidates: (1) map teacher_id on Class entity, " +
        "(2) fix ClassServiceImpl to set teacher_id on creation, " +
        "(3) replace native-SQL hasAccessToClass with JPA ownership check."
    )
    @DisplayName("A01-U01 [DISABLED — AUDIT FINDING]: Teacher-2 cannot GET attendance roster for a class owned by Teacher-1")
    void teacher2_cannotGetAttendance_forClassOwnedByTeacher1() {
        // Intentionally empty — see @Disabled Javadoc above for full audit finding.
        // This test cannot be implemented until:
        //   (a) Class entity maps teacher_id, AND
        //   (b) ClassServiceImpl.createClass() sets teacher_id, AND
        //   (c) Either Flyway is enabled in test profile OR ddl-auto generates the column.
    }
    ...
    @Test
    @Disabled(
        "AUDIT FINDING A01-CLASS-01 (same as A01-U01): classes.teacher_id absent from test schema; " +
        "ClassServiceImpl never sets teacher_id so the guard would deny the owner too. " +
        "Positive-path baseline cannot be established until production defect is fixed."
    )
    @DisplayName("A01-U03 [DISABLED — AUDIT FINDING]: Class owner (Teacher-1) CAN GET attendance roster — positive-path baseline")
