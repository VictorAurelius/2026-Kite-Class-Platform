/**
 * Attendance route-group layout — RoleGuard for OWNER/STAFF school-management.
 *
 * Wave RBAC-Shell 1 Bucket B (GAP-1130): SECURITY fix (IDOR-by-navigation).
 * Before this layout, `(dashboard)/attendance/*` only inherited the outer
 * `(dashboard)` auth check, so ANY authenticated user (teacher / parent /
 * student) could navigate straight to `/attendance` by typing the URL — the same
 ...
 * (Teacher attendance lives under the guarded `(teacher)` group, not here.)
 *
