...
**Reviewer-Approver:** @nguyenvankiet (solo-dev — MINOR self-approve per `rule-change-process.md` §5; new rule with built-in enforcement (PR template checkbox + memory auto-load + reviewer-checklist + worked self-test on Phase 3 staging.1-7 saga) per §6.5 Enforcement Parity Mandate; no constraint loosening for prior work; existing fix-PRs grandfathered, rule applies prospectively)
**Applies to:** Every release-tag retry sequence (e.g. `v0.9.0-beta-staging.N`, `v1.0.0-rc.N`) where a CI gate (Trivy / Lighthouse / E2E / smoke / strict-warnings) blocks tag promotion. Scope explicitly excludes fix-cycles inside a single PR (those follow normal review).
