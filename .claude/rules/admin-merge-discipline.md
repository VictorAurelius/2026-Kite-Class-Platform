...
**Reviewer-Approver:** @nguyenvankiet (solo-dev — v1.0.3 PATCH self-approve per `rule-change-process.md` §5; Wave meta-3 closure 2026-05-25 sync §11 Log v1.0.2 entry — GAP-735 now truly DONE (was PARTIAL when v1.0.2 documented prospective closure). Wave meta-3 empirical investigation per `release-fix-retry-budget.md` §3.5 confirmed 2 residual fails are service-layer functional bugs (`EnrollmentRepository.findByIdAndDeletedFalse` missing tenant filter) separable concern → re-classified GAP-746 P1; NOT in the "preexisting flaky tests kiteclass-core" exception class. `ADMIN_MERGE_OVERRIDE: GAP-735` trailer no longer needed prospectively. Future legitimate `--admin` use governed by §2 (post-rebase wait) + §4 (override trailer cho infra-dep / Lighthouse / smoke cases). No constraint change; factual sync. v1.0.2 (kept): PATCH documents systemic fix in-progress. v1.0.1 (kept): PATCH clarifies §4 trailer mechanism. v1.0.0 (kept): MINOR new rule.)
...
|---|---|
| `kitehub/{kitehub-admin,branding,email,gateway,platform,subscription}/**` | `cd kitehub && ./mvnw -pl <module> verify -P strict-warnings` |
| `kiteclass/kiteclass-core/**` | `cd kiteclass/kiteclass-core && ./mvnw verify -P strict-warnings` |
| `kitehub/kitehub-frontend/**` OR `kiteclass/kiteclass-frontend/**` | `pnpm -F <pkg> test --run && pnpm -F <pkg> build && pnpm -F <pkg> lint` |
...
git -C /tmp/wt-d push --force-with-lease
cd /tmp/wt-d/kitehub && ./mvnw -pl kitehub-subscription verify -P strict-warnings  # ← MUST PASS
...
**Apply §3 matrix:** Diff touches `kitehub/kitehub-subscription/**` → required `cd kitehub && ./mvnw -pl kitehub-subscription verify -P strict-warnings`.
...
1. Run `cd kitehub && ./mvnw -pl kitehub-subscription verify -P strict-warnings`
...
- **2026-05-25 (v1.0.3):** PATCH — Wave meta-3 closure sync. GAP-735 now truly 🟢 DONE (was PARTIAL when v1.0.2 documented prospective closure). Wave meta-3 closure PR (this same-day session) shipped 4-target sync: GAP-735 + GAP-745 flipped DONE + git mv → phase-1-beta/closed/; GAP-746 re-classified P2→P1 multi-tenant functional bug (separable concern from "preexisting flaky tests kiteclass-core" exception class). Empirical investigation per `release-fix-retry-budget.md` §3.5 (newly-landed v1.2.0 §3.5 Investigation phase mandate, PR #1821) revealed 2 residual fails are service-layer functional bugs in `EnrollmentRepository.findByIdAndDeletedFalse` (missing tenant filter) + `InvoiceServiceImpl` own-tenant filter — NOT test infra. ADMIN_MERGE_OVERRIDE: GAP-735 trailer no longer needed prospectively. Future legitimate `--admin` use governed by §2 (post-rebase wait) + §4 (override trailer for infra-dep / Lighthouse / smoke cases). Future GAP-746 multi-tenant fix wave can use ADMIN_MERGE_OVERRIDE: GAP-746 if cross-tenant tests block unrelated PRs (separate exception class with its own scope rationale). Reviewer: @nguyenvankiet (solo-dev PATCH self-approve per `rule-change-process.md` §5 — documentation-only Log entry sync with shipped 4-target closure; no constraint change).
- **2026-05-25 (v1.0.2):** PATCH — documents GAP-735 closure as systemic fix cho "preexisting flaky tests kiteclass-core" exception class. GAP-735 (filed 2026-05-24 PR #1775; closed Wave meta-1 Bucket A 2026-05-25) addressed 6 deterministic + suite-pollution test failures (`EnrollmentIntegrationTest` + `InvoiceFlowIntegrationTest` + `CourseSecurityTest` 4 cases) on `main` HEAD blocking every `kiteclass-core` PR via Test Core Service gate. Pre-fix PRs invoked `ADMIN_MERGE_OVERRIDE: GAP-735 — preexisting flaky tests` trailer per §4 — PR #1768 (Wave br-2 Bucket B authz), PR #1769 (Wave br-2 Bucket A), PR #1784 (Wave br-4 Bucket C pricingModel), PR #1787 (Wave br-4 Bucket D ClassMapper), PR #1788 (Wave br-4 strict-warnings hotfix). With GAP-735 closed, `ADMIN_MERGE_OVERRIDE:` trailer citing GAP-735 reason is NO LONGER NEEDED prospectively — kiteclass-core full suite passes clean on main HEAD. Future legitimate uses of `--admin` flag remain governed by §2 (post-rebase wait) + §4 (override trailer cho infra-dep / Lighthouse / smoke cases). Workflow `.github/workflows/core-ci.yml` không cần edit — generic CI without GAP-735-specific bypass logic. Reviewer: @nguyenvankiet (solo-dev PATCH self-approve per `rule-change-process.md` §5 — documentation-only Log entry sync with shipped systemic fix, no constraint change).
