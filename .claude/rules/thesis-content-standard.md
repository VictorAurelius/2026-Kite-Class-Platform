...
**Reviewer-Approver:** @nguyenvankiet (solo-dev — v1.0.1 PATCH self-approve per `rule-change-process.md` §5; Wave 102.1 user-flagged extension — adds §3 banned pattern + §C5 sub-criterion "Thesis is standalone academic deliverable. KHÔNG reference internal repo `documents/**` paths trong body narrative — committee không thấy được repo. Bibliography refs CHỈ từ sources công khai uy tín (academic papers / standards / official vendor docs / regulator docs). KHÔNG cite `documents/02-architecture/*.md` / `documents/04-quality/audits/*.md` / chapter MD cross-refs". No constraint loosening — codifies previously-implicit standalone-document principle exposed bởi Wave 102.1 fix scope. v1.0.0 (kept): new rule với built-in enforcement (9-category rubric grounded explicitly in (a) UTC spec PDF + (b) BAO_CAO sample + (c) DE_CUONG sample + (d) 43 persona-simulation agent findings + (e) 7 user-flagged content quality issues per §6.5 Enforcement Parity Mandate); state-check meta-miss "không tham khảo word-report trước à?" acknowledged mid-design; ≥8th recurrence per `feedback_outside_in_recurring_miss.md` memory.)
