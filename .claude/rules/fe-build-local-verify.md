...
**Applies to:** Mọi PR đụng frontend source dưới `kitehub/kitehub-frontend/**` hoặc `kiteclass/kiteclass-frontend/**` (`*.tsx`/`*.ts` trong `src/app/**`, `src/components/**`, `src/lib/**`, hoặc bất kỳ React/Next.js source). Out-of-scope: docs-only change trong FE folder (`*.md`), asset-only change (PNG/SVG), config-comment-only edit.
...
Pre-merge review cho PR đụng `kitehub/kitehub-frontend/**` hoặc `kiteclass/kiteclass-frontend/**` source:
