...
This rule closes the silent-loss path that surfaced 2026-04-27 in GAP-235 Sub-PR G: live screenshot capture was "deferred to manual run" yet the gap was still flipped to DONE in the closing commit. The deferred work has no enforcement mechanism — it relies on someone remembering — which is exactly the failure mode this rule prevents.
...
2. **No banned phrase** appears in the new Log entry without a paired follow-up gap reference. Banned phrases (case-insensitive):
   ...
   - `local can't`, `local doesn't`, `WSL2 too slow`, `chưa boot được`
   - `partial`, `partially`, `Phần X chưa` — when status is DONE
...
4. **If the gap had a wave plan or a multi-stage Proposed Fix, every stage is shipped** and referenced by PR number in the Log. PR-G-style "scope reduced because X" entries must include both the reduction reason AND the follow-up gap that catches the deferred remainder.
...
### Bad — what 2026-04-27 GAP-235 Sub-PR G originally tried
...
```markdown
**Status:** 🟡 PARTIAL — 5/6 sub-tasks DONE; live screenshot capture pending GAP-244 dev-stack fix

## Acceptance Criteria
- [x] OpenAPI spec exported
- [x] 10 v2 endpoints mocked
- ...
- [ ] Screenshots captured all 6 lifecycle states  →  blocked by GAP-244 (dev-stack boot)

## Log
- **2026-04-27** ... Live screenshot capture blocked by dev-stack schema mismatch (GAP-244 filed). Status stays PARTIAL until GAP-244 lands and capture is run.
```
...
| Item | Where |
| Live screenshot artifacts | GAP-XYZ — after GAP-244 unblocks dev stack |
```
...
This is the same failure mode that produced GAP-225 (scaffold-as-DONE governance debt) — five gaps shipped Wave 2-4 with explicit deferred items but DONE status. Cleaning that up cost real time.

---

## 6. Enforcement

- **Pre-merge automated:** `session-docs-check` skill **Rule 13** (paired same-PR with this rule) detects every `+**Status:** ... 🟢 DONE` line in the diff and runs the §2 checks; failure → BLOCK in `--strict` mode, WARN otherwise. See `.claude/skills/workflow/session-docs-check/scripts/check-docs.sh`.
...
- **`output-review-mandate.md`** §3 — this rule sharpens the DONE side of the review standard; matrix row "Gap closure (Status flip → DONE)" added v1.1.0 references this rule directly
- **`incident-to-rule-pipeline.md`** (sibling, same PR) — this rule is the first concrete output of that pipeline applied to GAP-235 Sub-PR G silent-deferral incident
- **`rule-change-process.md`** §6.5 (Enforcement Parity Mandate, same PR) — formalizes the rule+detection-same-PR pattern this rule demonstrates (paired with `session-docs-check` Rule 13)
- **`audit-to-gap-pipeline.md`** Step 2.5 — state-check at file-time; this rule is the matching state-check at close-time
- **`meta-gap-priority.md`** — meta gaps that fix this discipline (e.g. extending check-docs.sh) get meta-boost
