...
Verify:
```bash
aws secretsmanager list-secrets --query 'SecretList[?starts_with(Name,`kitehub/production/`)].[Name,RotationEnabled]' --output table
```
...
Verify:
```bash
aws secretsmanager describe-secret --secret-id kitehub/production/<name> \
...
- `kitehub-subscription` uses `kitehub/production/db-subscription` (not shared `db-master`)
- `kitehub-email` uses `kitehub/production/db-email` separately
- `kiteclass-core` uses `kiteclass/production/db-core` (tenant DBs separate)
