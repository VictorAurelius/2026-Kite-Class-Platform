...
**Reviewer-Approver:** @nguyenvankiet (solo-dev — v1.2.0 MINOR self-approve per `rule-change-process.md` §5; tightens §4.5 target 5: session handoff must be available on canonical `main`, not merely current working branch. Triggered by 2026-06-03 gpt-5.5 handoff miss: /start-session read working-branch latest handoff and missed `main:documents/03-planning/session-handoffs/2026-06-03-wave-14-gpt5.5-test-handoff.md`. v1.0.1 PATCH kept: stdin JSON statusline clarification. v1.0.0 kept: original context-% check rule.)
...
| 5 | `documents/03-planning/session-handoffs/YYYY-MM-DD-*.md` (added v1.1.0, tightened v1.2.0) | Session handoff note exists cho session date với scope shipped + pickup state cho next session AND is available on canonical `main` — `git ls-tree -r --name-only main -- documents/03-planning/session-handoffs \| grep "$(date +%Y-%m-%d).*\.md"` shows latest handoff. Local working branch handoff alone is NOT enough. |
...
- **2026-06-03 (v1.2.0):** MINOR — tightened §4.5 target 5: session handoff must be available on canonical `main`, not merely current working branch. Paired same-session update to `/start-session` skill Step 2: always read latest `documents/03-planning/session-handoffs/*.md` from `main` via `git ls-tree` + `git show main:<path>`. Triggered by user-flagged miss 2026-06-03: working branch latest handoff was `2026-06-02-wave-7-10-mega-session.md`, but canonical main had newer `2026-06-03-wave-14-gpt5.5-test-handoff.md` containing gpt-5.5 standalone test plan; assistant missed it during `/start-session`. Per `incident-to-rule-pipeline.md` 5-stage: Detect ✓ (user flagged "handoff cần nhất ở main") → Classify ✓ (existing session-end rule required local handoff exists, start-session skill did not mandate main-source handoff read) → Rule+Enforce ✓ (rule §4.5 tightened + start-session Step 2 updated) → Self-Test ✓ (`git show main:...2026-06-03-wave-14-gpt5.5-test-handoff.md` surfaces gpt-5.5 watch list; working branch read missed it) → Retro Log ✓. Reviewer: @nguyenvankiet (solo-dev MINOR self-approve per `rule-change-process.md` §5 — tightens session pickup gate; no constraint loosening).
