...
- **2026-05-21 (v1.0.4):** PATCH — Wave 102.7.5 meta cleanup: narrow `paths:` từ broad `documents/**/*.md` + `**/README.md` xuống 5 diagram-likely subdirs (architecture + diagrams + planning + rules + skills). Diagrams concentrate trong architecture docs + wave plans + rule/skill docs; broad pattern fired trên mọi document/README read without diagram relevance. Sync `rules-index.csv`. No constraint change; §2 selection matrix + §3 examples + §4 anti-patterns unchanged. Reviewer: @nguyenvankiet (solo-dev PATCH self-approve per `rule-change-process.md` §5 — path-scope narrowing per `context-budget-mandate.md` §3.1 broad-glob anti-pattern).
