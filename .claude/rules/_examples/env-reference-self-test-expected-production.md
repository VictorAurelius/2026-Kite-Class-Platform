...
Primary domain: `kitehub.me`. Support contact: `support@kitehub.me`.

Secrets Manager prefix: `kitehub/production/<service-name>` per service (e.g., `kitehub/production/jwt-signing-key`).

ECR registry: `906286017800.dkr.ecr.ap-southeast-1.amazonaws.com` (push/pull Docker images via `aws ecr get-login-password`).

Deploy IAM role ARN: `arn:aws:iam::906286017800:role/kitehub-deploy-role` (assumed via OIDC by `.github/workflows/terraform-apply.yml`).

CloudTrail trail name: `kitehub-main` (verified `IsLogging=true` pre-apply per `aws-observability-first.md` mandate).

## Sample database section

RDS database name: `kitehub`
RDS master user: `kitehub_prod`
