...
2. **What I should have said the first time:**
   > "Tôi sửa `settings-store.json` để add `kite-dev` vào `IntegratedWslDistros`, rồi restart Docker Desktop qua `taskkill.exe` + `cmd.exe`. Verify bằng `docker ps`."
