...
| **Security baseline** | security-audit skill /100 + **v2 audit format mandatory per GAP-564** (per-control evidence block: Command run + Output + Verdict + Evidence artifact ID — see `.claude/skills/quality/security-audit/reference/audit-report-template-v2.md`) | Post-wave + quarterly | Auditor | ✅ REFRESHED (2026-05-08, **87/100 B** — Wave 40 milestone, PR #974; +3 vs Wave 35 baseline 84; ✅ **PASS Phase 7 cổng ≥80**; pnpm 0 CVE cả 2 FE; Trivy active; P0=0, P1=3). v2 format takes effect Wave 80+ per GAP-564 (Wave 80 Bucket A). Wave 78 5 audit reports retrospectively annotated as "v1 format" (cost-benefit không re-run). |
