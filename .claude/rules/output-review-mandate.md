...
| **User-facing flow G1 browser-walk readiness** (Flow Verification Campaign §4 — flip flow CÓ FE từ chưa-G1 → `🔄 walk-pass-pending-human` PHẢI có ≥1 browser-real walk qua FE :3000, không chỉ curl `:9000` gắn header tay — bắt FE↔gateway contract / tenant-resolution / routing drift mà curl-with-manual-header che mất) | `.claude/rules/g1-browser-walk-before-flip.md` §3 6-row browser-walk evidence (FE :3000 thật + console clean + Network 2xx + FE-injected header observed + route resolves + ≥1 sad path) saved vào wave plan / campaign §4 row | Pre-flip reviewer-checklist §7.1 + path-scoped auto-load `wave-*-flow-*.md` + `flow-verification-campaign.md` + `*-g2-recipe-*.md`; campaign §1 G1 gate cross-ref; CI detector HONEST-deferred per `incident-to-rule-pipeline.md` §3.1 (recurrence 1) | G1-walker + reviewer + memory auto-load | ✅ DONE (2026-06-08 — Flow Verification Campaign KC-1 G2: rule v1.0.0 + memory + worked self-test §6 on KC-1 G2 — 3/3 bugs GAP-1067/1068/1069 would have surfaced ở G1 với browser-walk; META P1 force-multiplier prospective mọi FE flow G1 flip subsequent auto-comply) |
