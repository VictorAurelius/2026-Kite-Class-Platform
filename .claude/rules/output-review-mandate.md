...
| **Design-source implementation parity** (port Claude Design / Figma / mockup → ui_kits / production: every affordance/control/state present OR documented dropped) | `design-source-implementation-parity.md` v1.0.0 §3 parity checklist (copied-but-unwired grep) + reviewer-checklist | Pre-merge of PR porting design source into `documents/02-architecture/design-system/**` or frontend `src/**` | Author + reviewer | ⚠️ PARTIAL (2026-06-01 — rule + worked self-test marketing-site ThemeSwitcher dropped-wiring + same-PR fix; CI detector deferred per premature-rule guard) |
