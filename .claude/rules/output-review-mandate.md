...
| **Wave closure scope completeness** | `wave-closure-scope-completeness.md` §3 Scope-Completeness Reconciliation table + **§2.5 (v1.2.0) Quality-target wave gate** (wave tag/goal chứa score target → 0 gap OPEN/PARTIAL thuộc wave KHÔNG KỂ phase label + metric đạt trước flip complete; exception duy nhất = gap PENDING external-blocked; defer khác cần `WAVE_QUALITY_TARGET_DEFER:` user-approved) | At wave closure PR (draft → complete flip) + creation-time gate khai báo trong plan §5 | Author + reviewer + paired follow-up gap files | ✅ DONE (2026-05-18 v1.0.0 + Wave 92 self-test; v1.2.0 2026-06-11 quality-target gate không-kể-phase — landing-100 recurrence #3) |
...
| **Production FE port from kit** (production Next.js screen ported from / reconciled to HTML kit — kit→prod port OR prod→kit back-port) | `frontend-standards.md` §3.1 "Kit as Source of Truth" (kit-first workflow + kit↔production parity contract: visual/color-token/type/component/AC) | Pre-merge of FE PR touching `kiteclass-frontend/src/app/**` or `kitehub-frontend/src/app/**`; parity audit `quality/kit-production-parity/SKILL.md` 4-layer V-model | Author + reviewer (FE kit-parity PR-template checkbox) | ✅ DONE (2026-06-11 — Wave ui-kits-100 Bucket F: GAP-366 standard §3.1 + GAP-367 skill `kit-production-parity` 4-layer parity + PR template checkbox; GAP-1223 token-read-first lesson + #2326 bidirectional back-port) |
