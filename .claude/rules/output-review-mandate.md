...
| **Gap folder organization** (filesystem location mirrors CSV status+phase) | `gap-folder-organization.md` §2 9-subdir taxonomy + `scripts/check-gap-folder-location.sh` CI script (3 modes: --strict / --warn / --report-only) | Pre-merge of gap CRUD PR (create, status flip, phase reclassify) | Author + reviewer + CI script | ⚠️ PARTIAL (2026-05-18 — Wave 95 PR1: rule v1.0.0 + CI script in `--warn` mode + 9 subdir scaffolding shipped per `incident-to-rule-pipeline.md` 5-stage; PR2 mass migration of 347 misplaced files + flip to `--strict` mode queued; PR3 cross-link sweep queued per GAP-645 Bucket B/C/D — Wave 95 follow-up) |
