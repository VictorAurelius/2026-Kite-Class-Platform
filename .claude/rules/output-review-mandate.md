...
| **Wave closure scope completeness** | `wave-closure-scope-completeness.md` §3 Scope-Completeness Reconciliation table + **§2.5 (v1.1.0) Quality-target wave gate** (wave tag/goal chứa score target → 0 gap OPEN do wave tạo + metric đạt trước flip complete; defer cần `WAVE_QUALITY_TARGET_DEFER:` user-approved) | At wave closure PR (draft → complete flip) + creation-time gate khai báo trong plan §5 | Author + reviewer + paired follow-up gap files | ✅ DONE (2026-05-18 v1.0.0 + Wave 92 self-test; v1.1.0 2026-06-11 quality-target gate — landing-100 recurrence #3) |
