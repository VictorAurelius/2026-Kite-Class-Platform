...
**Reviewer-Approver:** @nguyenvankiet (solo-dev — v1.13.0 MINOR self-approve per `rule-change-process.md` §5; adds §3 matrix row "Session-end context check" tracking new rule `.claude/rules/session-end-context-check.md` v1.0.1 (paired same-PR Wave 100 prep 2026-05-19); no constraint loosening — adds coverage row for previously-uncovered output type "end-session decision moment". v1.12.3 (kept): see §11 Log for entries v1.12.3 .. v1.12.0 + [`_examples/output-review-mandate-log-history.md`](_examples/output-review-mandate-log-history.md) for entries v1.11.0 .. v1.0.0)
