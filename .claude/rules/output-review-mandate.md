...
**Reviewer-Approver:** @nguyenvankiet (solo-dev — MINOR self-approve per `rule-change-process.md` §5; new §3 matrix row "HTML/JSX prototypes" paired with same-PR enforcement: Wave UI Kits Round 2 will be the first kit set to apply this standard)
...
| **HTML/JSX prototypes** (`documents/02-architecture/design-system/ui_kits/**`) | Per-screen `/128` rubric (extends `quality/ui-review/SKILL.md` for static HTML path) + WCAG AA self-measurement in HTML comments + 100-item AC checklist (`design-system/dossier/10-acceptance-criteria.md`) | Pre-merge per kit PR self-report + user vibe-check after merge | Author self-review + user accepts | ⚠️ PARTIAL (Phase 1 standard documented 2026-04-29 GAP-263; Phase 2 ui-review-prototype skill extension → GAP-264; Phase 3 hook/CI enforcement → GAP-265) |
