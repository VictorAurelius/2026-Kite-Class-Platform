...
Verify: `grep -A5 'id: auth\|id: kitehub-auth' kitehub-gateway/.../application.yml` shows RequestRateLimiter on every entry.
...
When reviewing any PR that touches `kitehub-gateway/application.yml` OR `kitehub/kitehub-subscription/.../auth/**` OR adds new auth endpoint, reviewer asks: §2 checks for new endpoint still satisfied?
