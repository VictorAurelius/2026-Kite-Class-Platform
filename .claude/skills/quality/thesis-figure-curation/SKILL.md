...
description: "Dùng khi user nói 'figure curation', 'thesis figures', 'caption', 'đánh số hình', 'INDEX figure', 'curate figure cho Chapter N', 'add figure to thesis', 'kiểm tra caption hình', 'audit figure thesis'. Codifies selection criteria (figure vs table vs prose), per-chapter numbering N.M, Vietnamese caption format, per-chapter INDEX generation. Pair với GAP-646 thesis-docx pipeline."
---

# Thesis Figure Curation

Skill chuẩn hoá việc curate figure (hình ảnh) + table + diagram cho thesis V1+. Closes GAP-651 (image curation + per-chapter INDEX).

## Khi nào dùng

- Pre-merge PR thêm/sửa nội dung chapter thesis (`documents/08-thesis/chapter-*.md`)
...
- Trước khi assemble `thesis-vN.docx` (per GAP-646) — verify figure coverage + caption parity
...
5. **Generate INDEX** — tạo / update `documents/08-thesis/chapter-N-INDEX.md` từ audit output (table format § dưới)
...
## INDEX file format (`documents/08-thesis/chapter-N-INDEX.md`)
...
- **PNG paths** — hiện tại figure PNG nằm rải rác (`documents/08-thesis/figures/**`, `documents/06-diagrams/plantuml/*.png`, screenshots). Skill này KHÔNG migrate PNG files (out of scope) — chỉ index những gì có sẵn
...
- Gap closing: `documents/04-quality/gaps/phase-1-beta/closed/GAP-651-thesis-image-curation.md`
- Sister gap: GAP-646 thesis-docx-pipeline (figure injection `{{figure:N.M}}` marker)
- Sister gap: GAP-648 thesis-nfr-data-capture (CloudWatch screenshots feed curation)
- Rule: `.claude/rules/diagram-format-selection.md` — Mermaid mặc định, ASCII ≤5 nodes, PlantUML cho C4
- Rule: `.claude/rules/thesis-content-standard.md` C7 Diagram+figure rendering category
- Rule: `.claude/rules/vn-localization-audit-checklist.md` §2 — VN sample data + format conventions
