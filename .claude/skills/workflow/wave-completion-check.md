...
## Gotchas
- **Level 0 state-check is non-negotiable AFTER GAP-235 incident** — every gap claimed `🟢 DONE` in the wave must satisfy `.claude/rules/gap-done-discipline.md` §2 (every AC checkbox `[x]`, no banned phrases like `deferred`/`manual run`/`partial` in the close Log); a wave passing Level 1-7 with a false-DONE gap is still a failed wave (memory `feedback_post_merge_doc_sync.md`)
- **Level 7 audit freshness window is 7 days, NOT post-wave only** — `audit-gate.py` BLOCKS follow-up PRs if a required audit is >7 days stale; `.claude/rules/post-wave-audit-mandate.md` mandates audit suite within 3 days of wave merge or hook starts blocking domain-touching PRs (Wave 5 incident 2026-04-25 — 5 audits ran in single Sub-PR 5.6a)
- **Stacked PR `--delete-branch` auto-closes children** — when merging wave → main with `--delete-branch`, any unmerged stacked agent PR pointing at the wave branch gets auto-closed AND cannot be reopened (memory `feedback_stacked_pr_delete_branch.md`); merge wave WITHOUT `--delete-branch`, drain remaining agent PRs to main, THEN clean up the wave branch
- **`gh pr checks --json state` returns null** — Level 1 polling loops based on `state` field never exit; use `scripts/check-ci.sh <branch> --status` or query `--json status` instead (memory `feedback_monitor_gh_pr_checks_no_state_field.md`)
- **Wave plan doc must merge via PR before agents spawn** — pushing `documents/03-planning/waves/wave-X-*.md` directly to main "because agents need it ready" violates CLAUDE.md and corrupts the wave audit trail (incident 2026-04-26, memory `feedback_wave_plan_through_pr.md`); plan PR is the wave's first PR, not a precursor
