...
- **INLINE-HYBRID LUÔN NHẮC (per `agent-concurrency-budget-inline-hybrid.md`)** — mọi session, mọi turn có agent đang chạy: KHÔNG ngồi chờ. Spawn ÍT agent (heavy-cohesive bucket, tránh server/rate limit) + LẤP idle bằng inline bucket disjoint chạy SONG SONG agents → tối ưu wall-clock liên tục. Sau mỗi spawn, tự hỏi "còn bucket inline-able (disjoint với agent) không?" → làm ngay. "ít agent" PHẢI đi kèm "coordinator làm inline", KHÔNG phải "ngồi chờ". Anti-pattern 2026-06-21: spawn agent rồi báo "chờ agent xong" thay vì làm bucket disjoint inline. Collect-state.sh §Ghi chú in nhắc này mỗi session start.
